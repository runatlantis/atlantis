...
[
  {
    ...
    "ReqApprovalCount": 1,
    "Approvals": null,
    "Hashes": ["FAIL - plan.json - deny resource creation"]
  }
]
```
...
| `Hashes`           | []string            | Items extracted from the policy output using `policy_item_regex`.           |
...
When sticky approvals are enabled, Atlantis extracts "hash items" from policy output using a configurable regex (`policy_item_regex`). Each approval records a snapshot of these hashes. On re-plan, previous approvals are carried forward only if their recorded hashes still cover the current policy output.

If the policy output changes in a way that introduces items not present at approval time (e.g., a new violation appears or an existing one changes), approvals whose hashes no longer cover the current output are dropped, and re-approval is required. Removing items (e.g., fixing a violation) does not invalidate existing approvals.
...
The `policy_item_regex` controls which parts of the policy output are used as the "identity" for sticky approval tracking. The default is `.*`, which extracts every line individually (Go's `.*` does not match across newlines). An approval carries over when every line in the current output was also present in the output at approval time -- in other words, the current items must be a subset of the approved items. Adding or changing a line invalidates the approval, but removing a line (e.g. fixing a violation) preserves it.

For more targeted matching, use a regex that captures only the meaningful failure lines:
...
With `(?m)^FAIL.*`, only lines starting with `FAIL` are tracked. Approvals remain valid as long as every current `FAIL` line was present at approval time, even if other parts of the output (like summary counts) differ. Fixing a failure (removing a `FAIL` line) preserves existing approvals; adding or changing one invalidates them.
