...
* `--auto-merge-method method` Specify which [merge method](automerging.md#how-to-set-the-merge-method-for-automerge) to use for the apply command if [automerge](automerging.md) is enabled. The accepted values depend on the VCS provider.
* `--verbose` Append Atlantis log to comment.
