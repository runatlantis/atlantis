...
| `CHECK_TYPE`         | Indicates which authorization phase is being evaluated. `pre_flight` for the early repo-level check (before clone/project discovery), `project` for the per-project check (after project discovery). Use this to apply different logic when workspace/project context is unavailable. |
| `COMMAND_NAME`       | The name of the command that is being executed, i.e. `plan`, `apply` etc.                                                                                                                                                             |
| `PROJECT_NAME`       | Name of the project. During `pre_flight` checks, only set if the user explicitly specified `-p` in the command. Always set during `project` checks.                                                                                   |
| `USER_NAME`          | Username of the VCS user running command, ex. `acme-user`. During an autoplan, the user will be the Atlantis API user, ex. `atlantis`.                                                                                                |
| `WORKSPACE`          | The Terraform workspace. During `pre_flight` checks, only set if the user explicitly specified `-w` in the command. Always set during `project` checks.                                                                               |
