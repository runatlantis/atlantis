...
Obtén la última release desde [GitHub](https://github.com/runatlantis/atlantis/releases)
y descomprímela.
...
Una forma de lograr esto es con ngrok, una herramienta que reenvía tu puerto local a un hostname público
aleatorio.

[Descargar](https://ngrok.com/download) ngrok y `unzip`.
...
<details>
    ...
    <ul>
        ...
        <li>No marques <strong>Skip certificate validation</strong> porque NGROK tiene un certificado válido.</li>
...
<details>
    ...
    <ul>
        ...
        <li>verifica de nuevo que agregaste <code>/events</code> al final de tu URL.</li>
        <li>establece <strong>Secret</strong> en el secreto de webhook que generaste previamente
        <ul>
        ...
        <li>Deja <strong>Active</strong> marcado</li>
        <li>Haz clic en <strong>Add Webhook</strong></li>
        <li>Consulta <a href="#next-steps">Próximos pasos</a></li>
    </ul>
</details>
...
- Elige una opción de vencimiento **Do not expire** o **Expire automatically**
...
O simplemente modifica el espacio en blanco en un archivo.
:::
...
Si quieres `apply`, escribe un comentario: `atlantis apply`. Puedes usar las flags `-d` o `-w` para apuntar
...
- Con suerte Atlantis está funcionando con tu repo y estás listo para continuar con un [despliegue listo para producción](../docs/deployment.md).
- Si no está funcionando como se espera, puede que necesites personalizar cómo Atlantis se ejecuta con un archivo `atlantis.yaml`.
Consulta [atlantis.yaml use cases](../docs/repo-level-atlantis-yaml.md#use-cases).
- Consulta nuestra [documentación completa](../docs.md) para más detalles.
