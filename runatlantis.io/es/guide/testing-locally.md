...
`terraform` debe estar en el `$PATH` para Atlantis.
...
Atlantis necesita ser accesible en algún lugar al que github.com/gitlab.com/bitbucket.org o tu instalación de GitHub/GitLab Enterprise puedan llegar.
Una manera de lograr esto es con ngrok, una herramienta que reenvía tu puerto local a un hostname
público aleatorio.

[Download](https://ngrok.com/download) ngrok y `unzip`.

Inicia `ngrok` en el puerto `4141` y toma nota del hostname que te da:

```bash
./ngrok http 4141
```

En una pestaña nueva (donde pronto iniciarás Atlantis) crea una variable de entorno con
el hostname de ngrok:

```bash
URL="https://{YOUR_HOSTNAME}.ngrok.io"
```

## Crear un Webhook Secret

GitHub y GitLab usan webhook secrets para que los clientes puedan verificar que los webhooks
provienen de ellos.
...
<details>
    ...
    <ul>
        ...
        <li>Haz clic en <strong>Add webhook</strong></li>
        <li>Ingresa "Atlantis" para <strong>Title</strong></li>
        <li>Establece <strong>URL</strong> en tu URL de ngrok con <code>/events</code> al final. Ej. <code>https://c5004d84.ngrok.io/events</code></li>
        <li>Verifica de nuevo que agregaste <code>/events</code> al final de tu URL.</li>
        ...
        <li>Bajo <strong>Repository</strong><strong>un</strong>marca todo</li>
        <li>Bajo <strong>Issues</strong> deja todo <strong>un</strong>marcado</li>
...
<details>
    ...
    <ul>
        ...
        <li>Ingresa "Atlantis" para <strong>Name</strong></li>
        <li>Establece <strong>URL</strong> en tu URL de ngrok con <code>/events</code> al final. Ej. <code>https://c5004d84.ngrok.io/events</code></li>
        <li>Verifica de nuevo que agregaste <code>/events</code> al final de tu URL.</li>
        <li>Establece <strong>Secret</strong> en tu cadena aleatoria</li>
...
<details>
    ...
    <ul>
        ...
        <li>verifica de nuevo que agregaste <code>/events</code> al final de tu URL.</li>
        <li>establece <strong>Secret</strong> en el Webhook Secret que generaste previamente
        <ul>
            <li><strong>NOTE</strong> Si estás agregando un webhook a múltiples repositorios, cada repositorio necesitará usar el <strong>mismo</strong> secret.</li>
        </ul>
        </li>
...
## Crear un access token para Atlantis
...
puedes usar tu propio usuario. Aquí crearemos el access token que Atlantis usa para comentar en el pull request y
establecer estados de commit.

### Access Token de GitHub o GitHub Enterprise
...
### Access Token de GitLab o GitLab Enterprise
...
### Access Token de Bitbucket Cloud (bitbucket.org)
...
### Access Token de Bitbucket Server (también conocido como Stash)
...
- Dale al token permisos de proyecto **Read** y permisos de pull request **Write**
- Elige una opción de Expiry **Do not expire** o **Expire automatically**
...
::: tip
Podrías agregar un recurso null como prueba:
...
Deberías ver a Atlantis registrando que recibió el webhook y deberías ver la salida de `terraform plan` en tu repo.

Atlantis intenta averiguar el directorio en el que hacer plan según los archivos modificados.
...
Para ejecutar manualmente `plan` en un directorio o workspace específico, comenta en el pull request usando las flags `-d` o `-w`:
...
Si quieres ejecutar `apply`, escribe un comentario: `atlantis apply`. Puedes usar las flags `-d` o `-w` para apuntar
Atlantis a un plan específico. De lo contrario intenta aplicar el plan para el directorio raíz.

## Logs en tiempo real

La [salida de terraform en tiempo real](../docs/streaming-logs.md) para tu comando se puede encontrar haciendo clic en la verificación de estado para un proyecto dado en un PR, que
enlaza con la UI de transmisión de logs. Esta es una UI de terminal donde puedes ver tus comandos ejecutándose en tiempo real.

## Próximos pasos

- Si las cosas están funcionando como se espera puedes `Ctrl-C` el comando `atlantis server` y el comando `ngrok`.
- Con suerte Atlantis está funcionando con tu repo y estás listo para continuar con un [deployment listo para producción](../docs/deployment.md).
...
Consulta [casos de uso de atlantis.yaml](../docs/repo-level-atlantis-yaml.md#use-cases).
