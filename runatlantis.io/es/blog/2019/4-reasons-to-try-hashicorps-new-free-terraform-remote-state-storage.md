---
title: 4 razones para probar el (nuevo) almacenamiento remoto de estado gratuito
  de Terraform de HashiCorp
lang: en-US
---

# 4 razones para probar el (nuevo) almacenamiento remoto de estado gratuito de Terraform de HashiCorp
...
HashiCorp está planeando ofrecer Terraform Remote State Storage gratuito y ya tienen una versión beta disponible. En este artículo, hablo de 4 razones por las que deberías probarlo (Divulgación: trabajo en HashiCorp).

> _[Regístrate en Terraform Cloud](https://goo.gl/X5t5EM)._

## ¿Qué es Terraform State?

Antes de entrar en por qué deberías usar el nuevo almacenamiento remoto de estado, hablemos de qué queremos decir exactamente con state en Terraform.
...
De forma predeterminada, Terraform escribe su archivo de state en tu sistema de archivos local. Esto está bien para proyectos personales, pero una vez que empiezas a trabajar con un equipo, las cosas se complican. En un equipo, necesitas asegurarte de que todos tengan una versión actualizada del archivo de state **y** garantizar que dos personas no estén haciendo cambios concurrentes.

¡Aquí entra remote state! Remote state es simplemente almacenar el archivo de state de forma remota, en lugar de hacerlo en tu sistema de archivos. Con remote state, solo hay una copia, por lo que Terraform puede garantizar que siempre estés actualizado. Para evitar que miembros del equipo modifiquen el state al mismo tiempo, Terraform puede bloquear el remote state.

> Remote state es simplemente almacenar el archivo de state de forma remota, en lugar de hacerlo en tu sistema de archivos.

Bien, entonces remote state es excelente, pero desafortunadamente configurarlo puede ser un poco complicado. En AWS, puedes almacenarlo en un bucket de S3, pero necesitas crear el bucket, configurarlo correctamente, establecer correctamente sus permisos, crear una tabla de DynamoDB para el bloqueo y luego asegurarte de que todos tengan las credenciales adecuadas para escribir en él. Es prácticamente la misma historia en las otras nubes.
...
A diferencia de otras soluciones de remote state que requieren una configuración complicada para que funcione bien, configurar el almacenamiento remoto de state gratuito es fácil.
...
Si alguna vez has estropeado tu state de Terraform y has necesitado descargar una versión antigua o has querido un registro de auditoría para saber quién cambió qué, entonces te encantará esta funcionalidad.
...
Por supuesto, puedes encontrar una forma de obtener esta información desde algunos de los otros backends de state, pero es difícil. Con el almacenamiento remoto de state de HashiCorp, lo obtienes gratis.
...
Establece una variable de entorno `ATLANTIS_TFE_TOKEN` en un token de TFE y estarás listo. Ve a <https://www.runatlantis.io/docs/terraform-cloud.html> para obtener más información.

Conclusión
Te animo encarecidamente a probar el nuevo backend gratuito de Remote State Storage. Es una oferta convincente frente a otros backends de state gracias a su facilidad de configuración, su visor de state con todas las funciones y sus capacidades de bloqueo.
