...
Atlantis es una aplicación autohospedada que escucha eventos de pull request de Terraform a través de webhooks. Ejecuta `terraform plan` e `apply` de forma remota y comenta de vuelta en el pull request con la salida.
...
Como todo lo que estamos haciendo es agregar un recurso null, creo que es seguro ejecutar `terraform apply`. Para hacerlo, agrego un comentario al pull request: `atlantis apply`:
...
Además de poder `plan` e `apply` desde el pull request, Atlantis también te permite personalizar los comandos exactos que se ejecutan mediante un archivo de configuración `atlantis.yaml`. Por ejemplo, para usar el flag `-var-file`:
...
Atlantis evitará que otros pull requests se ejecuten contra el mismo directorio que un pull request abierto para que cada plan se aplique de forma atómica. Una vez que el primer pull request se fusiona, otros pull requests se desbloquean.
