...
Supongamos que tenemos dos desarrolladores: Alice y Bob. Alice necesita agregar una nueva regla de security group. Ella hace checkout de una nueva rama, agrega su regla y crea un pull request:
...
Ahora volvamos con Alice. Acaba de obtener aprobación para el cambio de su pull request y entonces ejecuta `terraform apply`:
...
- SIEMPRE crearan una rama, obtuvieran una revisión del pull request, hicieran merge a `master` y luego ejecutaran apply. Y también todos
- SIEMPRE verificaran que su rama fue rebaseada desde `master`. Y también todos
...
El problema central es que todos están aplicando desde sus propias estaciones de trabajo y depende de ellos asegurarse de que están actualizados y de mantener `master` actualizado. Esto es como si los desarrolladores desplegaran a producción desde sus laptops.
...
### Pero ¿qué pasaría si Bob ejecutara `apply` localmente?
...
Hacemos esto porque `terraform apply` falla con bastante frecuencia, a pesar de que `terraform plan` tenga éxito. Usualmente es por un problema de dependencia entre recursos o porque el proveedor cloud requiere un cierto formato o que se establezca un cierto campo. De cualquier modo, en la práctica hemos encontrado que `apply` falla mucho.

Al bloquear el directorio, esencialmente estamos asegurando que la rama a la que se le hace `apply` esté `"master"` ya que nadie más puede modificar ese estado. Luego obtenemos el beneficio de poder iterar en el pull request y hacer push de pequeñas correcciones hasta estar seguros de que el changeset fue `apply`'d. Si `apply` fallara después de hacer merge a `master`, tendríamos que abrir nuevos pull requests una y otra vez. Definitivamente hay un tradeoff aquí, sin embargo creemos que es el tradeoff correcto.

## Conclusión

En conclusión, ejecutar `terraform apply` cuando trabajas con un equipo de operadores puede ser peligroso. Busca soluciones como tu propio CI, Atlantis o Terraform Enterprise para asegurar que siempre estés trabajando sobre el código más reciente al que se le hizo `apply`.
