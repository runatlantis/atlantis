...
Elegí Terraform en este caso porque Atlantis es una herramienta para automatizar y colaborar en Terraform en un equipo (ver github.com/runatlantis/atlantis), y por lo tanto obviamente tenía sentido alojar nuestra página principal usando Terraform, pero también porque ahora es mucho más fácil de administrar. No tengo que entrar en la consola de AWS y hacer clic por todos lados para encontrar qué configuraciones quiero cambiar. En su lugar, puedo simplemente mirar ~100 líneas de código, hacer un cambio y ejecutar `terraform apply`.

::: info
NOTA: 4 meses después de escribir esto, moví el sitio a [Netlify](https://www.netlify.com/) porque construye automáticamente desde mi rama master ante cualquier cambio, actualiza más rápido ya que no necesito esperar a que expire la caché de Cloudfront y me da [deploy previews](https://www.netlify.com/blog/2016/07/20/introducing-deploy-previews-in-netlify/) de los cambios. Los registros DNS todavía están alojados en AWS.
:::

# Resumen

Hay una cantidad sorprendente de componentes requeridos para hacer que todo esto funcione, así que voy a empezar con un resumen de para qué se necesitan todos. Así es como se ve la arquitectura final:
...
El primer paso es tener un sitio generado. Nuestro sitio usa [Hugo](https://gohugo.io/), un generador de sitios de Golang. Una vez que está configurado, solo necesitas ejecutar `hugo` y generará un directorio con HTML y todo tu contenido listo para alojar.

## Paso 2 — Alojar el contenido

Una vez que tienes un sitio web, necesitas que sea accesible en internet. Usé S3 para esto porque es extremadamente barato y se integra bien con todos los otros componentes necesarios. Simplemente subo la carpeta de mi sitio web al bucket de S3.
...
Debido a que voy a alojar el sitio en servicios de AWS, necesito que las solicitudes a <www.runatlantis.io> se enruten a esos servicios. Route53 es la solución obvia.

## Paso 5 — Alojar con CloudFront

En este punto, hemos generado un certificado SSL para <www.runatlantis.io> y nuestro sitio web está disponible en internet a través de su URL de S3, así que ¿no podemos simplemente hacer un CNAME al bucket de S3 y dar por terminado el día? Desafortunadamente no.
...
Ahora que sabemos cómo debe verse nuestra arquitectura, es simplemente cuestión de escribir el Terraform.
...
Suponiendo que ya hemos generado el contenido de nuestro sitio, necesitamos crear un bucket de S3 para alojar el contenido.
...
Si compraste tu dominio en otro lugar como Namecheap, necesitarás apuntar tu DNS a los nameservers listados en el state de la zona de Route53 que creaste. Primero `terraform apply` (lo que puede tomar un tiempo), luego averigua tus nameservers.
...
Luego revisa la documentación de tu dominio para ver cómo cambiar tus nameservers a los 4 listados.
...
Resulta que necesitamos crear un bucket de S3, una distribución de CloudFront y un registro de Route53 completamente nuevos solo para lograr que esto suceda. Eso es porque aunque S3 puede servir una redirección a la versión www de tu sitio, no puede alojar certificados SSL y por lo tanto necesitas CloudFront. He incluido abajo todo el Terraform necesario para eso.
