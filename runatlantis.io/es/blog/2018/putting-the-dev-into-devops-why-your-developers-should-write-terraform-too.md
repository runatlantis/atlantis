...
Terraform no es solo una herramienta que hace que los equipos de ops sean más efectivos. Adoptar Terraform es una oportunidad para convertir a todos tus desarrolladores en operadores (al menos para tareas más pequeñas). Esto puede hacer que todo tu equipo de ingeniería sea más efectivo y crear una mejor relación entre desarrolladores y operadores.

### Aparte rápido — ¿Qué es Terraform?
...
Y es una herramienta CLI que lee código de Terraform y hace llamadas API a AWS (o a cualquier otro proveedor de nube) para aprovisionar esa infraestructura.
...
Adoptar Terraform es excelente para la efectividad de tu equipo de operaciones, pero no cambia mucho para los devs. Antes de adoptar Terraform, los devs normalmente interactuaban con un equipo de ops así:

![](../../../blog/2018/putting-the-dev-into-devops/pic1.webp)

1. **Dev: Crea un ticket pidiendo algo de trabajo de ops**
2. **Dev: Espera**
...
En muchas organizaciones, los devs no tienen acceso a la consola de nube.

![](../../../blog/2018/putting-the-dev-into-devops/pic3.webp)

Puede que no tengan acceso por buenas razones:
...
- A menudo es difícil hacer cosas aparentemente simples (piensa en agregar una regla de security group que también requiera peering de VPC). Esto significa que a veces solo tener acceso no es suficiente. Los devs podrían necesitar ayuda de un experto para lograr hacer las cosas.
...
O dónde se establece el tamaño del autoscaling group:
...
Los devs entienden el código (¡sorpresa!) así que para ellos es mucho más fácil hacer esos cambios pequeños.

Este es el nuevo workflow:
...
- Los devs están haciendo cambios pequeños por sí mismos. Esto ahorra tiempo y aumenta la velocidad de toda la organización de ingeniería.
- Los devs pueden ver exactamente qué se requiere para hacer el cambio. Esto significa que hay menos ida y vuelta sobre un ticket: “Bien, entonces sé que necesitas abrir el security group entre el servidor A y B, ¿pero en qué puertos y con qué protocolo?”
- Los devs empiezan a ver cómo se construye la infraestructura. Esto aumenta la cooperación entre dev y ops porque pueden entender el trabajo de los demás.

¡Genial! Pero hay otro problema.

## Los devs tampoco tienen acceso a Terraform

¡Para ejecutar Terraform necesitas credenciales de nube! Es realmente difícil escribir Terraform sin poder ejecutar `terraform init` e `terraform plan`, por la misma razón por la que sería difícil escribir código si nunca pudieras ejecutarlo localmente.
...
El desarrollador envía un nuevo commit que corrige su error y Atlantis responde con un comentario con la salida válida de `terraform plan`. Ahora el desarrollador puede verificar que la salida del plan se ve bien.

![](../../../blog/2018/putting-the-dev-into-devops/pic7.webp)

### Paso 4 — Obtener aprobación

Probablemente querrás ejecutar Atlantis con el flag --require-approval que requiere que los pull requests estén Approved antes de ejecutar atlantis apply.

![](../../../blog/2018/putting-the-dev-into-devops/pic8.webp)

### Paso 4a — De hecho obtener aprobación
...
Ahora los desarrolladores pueden hacer pequeños cambios de operaciones y aprender más sobre cómo se construye la infraestructura. Todos pueden trabajar de forma más efectiva y con un entendimiento compartido que mejora la colaboración.

## ¿Funciona en la práctica?

Atlantis ha sido usado por mi empresa anterior, Hootsuite, durante más de 2 años. Lo usan diariamente 20 operadores, ¡pero también lo usan ocasionalmente más de 60 desarrolladores!
...
- Si te gustaría aprender más sobre Terraform, revisa la [Introduction to Terraform](https://developer.hashicorp.com/terraform/intro) de HashiCorp
- Si te gustaría probar Atlantis, ve a <www.runatlantis.io>
- Si tienes alguna pregunta, contáctame en Twitter ([at]lkysow) o en los comentarios de abajo.
