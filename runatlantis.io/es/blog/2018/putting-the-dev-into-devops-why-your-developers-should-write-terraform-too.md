...
Terraform no es solo una herramienta que hace a los equipos de ops más efectivos. Adoptar Terraform es una oportunidad para convertir a todos tus desarrolladores en operadores (al menos para tareas más pequeñas). Esto puede hacer a todo tu equipo de ingeniería más efectivo y crear una mejor relación entre desarrolladores y operadores.
...
Y es una herramienta CLI que lee código de Terraform y hace llamadas de API a AWS (o cualquier otro proveedor de nube) para aprovisionar esa infraestructura.
...
Adoptar Terraform es excelente para la efectividad de tu equipo de operaciones pero no cambia mucho para los devs. Antes de la adopción de Terraform, los devs normalmente interactuaban con un equipo de ops así:
...
- A menudo es difícil hacer cosas aparentemente simples (piensa en agregar una regla de security group que también requiere peering de VPCs). Esto significa que el solo hecho de tener acceso a veces no es suficiente. Los devs podrían necesitar ayuda de un experto para hacer las cosas.
...
Los devs entienden código (¡sorpresa!) así que es mucho más fácil para ellos hacer esos pequeños cambios.
...
- Los devs están haciendo pequeños cambios ellos mismos. Esto ahorra tiempo e incrementa la velocidad de toda la organización de ingeniería.
...
- Los devs empiezan a ver cómo se construye la infraestructura. Esto incrementa la cooperación entre dev y ops porque pueden entender el trabajo del otro.
...
El desarrollador hace push de un nuevo commit que corrige su error y Atlantis comenta de vuelta con la salida válida de `terraform plan`. Ahora el desarrollador puede verificar que la salida del plan se vea bien.
...
Ahora un operador puede venir y revisar los cambios y la salida de `terraform plan`. Esto es mucho más rápido que hacer el cambio ellos mismos.
...
Ahora los desarrolladores pueden hacer pequeños cambios de operaciones y aprender más sobre cómo se construye la infraestructura. Todos pueden trabajar de forma más efectiva y con una comprensión compartida que mejora la colaboración.
...
- Gracias a Isha por leer los borradores de esta publicación.
- Íconos en los gráficos hechos por [Freepik](https://www.freepik.com/) de [Flaticon](https://www.flaticon.com/) y licenciados bajo [CC 3.0](https://creativecommons.org/licenses/by/3.0/)
