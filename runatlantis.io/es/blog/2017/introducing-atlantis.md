---
title: Presentación de Atlantis
lang: en-US
---

# Presentación de Atlantis
...
Estamos muy entusiasmados de anunciar el lanzamiento open source de Atlantis. Atlantis es una herramienta para
...
`apply` directamente desde pull requests de Terraform. Atlantis luego responde en el pull
...
Atlantis fue construido para resolver dos problemas que surgieron en Hootsuite mientras adoptábamos Terraform:
...
Entonces comenzamos a usar pull requests:
...
El segundo problema es que ahora es fácil que `master` se desincronice de lo que realmente se ha aplicado. Esto puede pasar si haces merge de un pull request sin ejecutar `apply` o si `apply` tiene un error a mitad del proceso, olvidas corregirlo y luego haces merge a `master`. Ahora lo que está en `master` no es en realidad lo que está ejecutándose en producción. En el mejor de los casos, esto causa confusión la próxima vez que alguien ejecute `terraform plan`. En el peor de los casos, causa una caída cuando alguien asume que lo que está en `master` realmente está ejecutándose, y depende de ello.

Con el workflow de Atlantis, estos problemas se resuelven:

![](../../../blog/2017/intro/intro6.webp)

Ahora es fácil revisar los cambios porque ves la salida de `terraform plan` en el pull request.
...
Terraform normalmente comienza siendo usado por el equipo de Ops. Como resultado de usar Terraform, el equipo de Ops se vuelve mucho más rápido para hacer cambios de infraestructura, pero la manera en que los desarrolladores solicitan esos cambios sigue siendo la misma: usan un sistema de tickets o chat para pedir ayuda a operaciones, la solicitud entra en una cola y más tarde Ops responde que la tarea está completa.

Sin embargo, pronto el equipo de Ops comienza a darse cuenta de que es posible que los desarrolladores hagan algunos de estos cambios de Terraform por sí mismos. Aunque surgen algunos problemas:

- Los desarrolladores no tienen las credenciales para ejecutar realmente los comandos de Terraform
- Si les das credenciales, es difícil revisar lo que realmente se está aplicando

Con Atlantis, estos problemas se resuelven. Todos los comandos `terraform plan` y `apply` se ejecutan desde el pull request. Esto significa que los desarrolladores no necesitan tener ninguna credencial para ejecutar Terraform localmente. Por supuesto, esto puede ser peligroso: ¿cómo puedes asegurarte de que los desarrolladores (que podrían ser nuevos en Terraform) no estén aplicando cosas que no deberían? La respuesta son revisiones de código y aprobaciones.

Como Atlantis responde con la salida de `plan` directamente en el pull request, es fácil para un ingeniero de operaciones revisar exactamente qué cambios se aplicarán. Y Atlantis puede ejecutarse en modo `require-approval`, que requerirá una aprobación de pull request de GitHub antes de permitir que se ejecute `apply`:
...
Si quieres probar Atlantis para tu equipo puedes descargar la última release desde <https://github.com/runatlantis/atlantis/releases>. Si ejecutas `atlantis testdrive` puedes comenzar en menos de 5 minutos. Para leer más sobre Atlantis ve a <https://www.runatlantis.io/>.
