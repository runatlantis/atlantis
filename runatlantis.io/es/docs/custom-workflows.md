...
Los workflows personalizados se pueden especificar en la Server-Side Repo Config o en los archivos de nivel de repositorio
`atlantis.yaml`.

**Notas:**

* Si desea permitir que los repos seleccionen sus propios workflows, deben tener la configuración
`allowed_overrides: [workflow]`. Vea [casos de uso de server-side repo config](server-side-repo-config.md#allow-repos-to-choose-a-server-side-workflow) para más detalles.
* Si además también desea permitir que los repos definan sus propios workflows, deben tener la configuración
`allow_custom_workflows: true`. Vea [casos de uso de server-side repo config](server-side-repo-config.md#allow-repos-to-define-their-own-workflows) para más detalles.
...
::: tip
Antes de crear workflows personalizados para archivos `.tfvars`, considere usar la característica automática de Atlantis de `env/{workspace}.tfvars`. Si estructura sus archivos como `env/staging.tfvars`, `env/production.tfvars`, etc., Atlantis los incluirá automáticamente según el workspace sin ninguna configuración. Vea [Using Atlantis - Automatic Environment Variable Files](using-atlantis.md#automatic-environment-variable-files) para más detalles.
:::
...
Si quisiera que Atlantis ejecutara automáticamente plan con `-var-file staging.tfvars` e `-var-file production.tfvars`
...
Luego, en su archivo `atlantis.yaml` de nivel de repositorio, referenciaría los workflows:
...
Cuando quiera aplicar los planes, puede comentar
...
Si necesita agregar flags a `terraform plan` o `apply` temporalmente, puede
agregar flags en un comentario después de `--`, por ejemplo comentando:
...
entonces debe definir un workflow personalizado y establecer la clave `extra_args` para el
comando que necesita modificar.
...
::: tip Nota
Cada entrada en `extra_args` se pasa a Terraform como un solo argumento. No es
interpretada por un shell, por lo que los operadores de shell (`;`, `&&`, `|`, redirecciones),
globbing y división de palabras no se aplican. Las referencias a variables de entorno tales
como `$WORKSPACE`, `$DIR` e `$ATLANTIS_TERRAFORM_VERSION` aún se expanden.

Si necesita comportamiento de shell, use un paso `run`, que se ejecuta con un shell por
diseño.
:::
...
Si quiere personalizar `terraform init`, `plan` o `apply` de formas que
no son compatibles con `extra_args`, puede sobrescribir completamente esos comandos.
...
**Nota:** no use el directorio predeterminado `cdktf.out` que usa CDKTF, ya que este debería estar en la lista `.gitignore` del
repositorio, de modo que los archivos generados localmente no se confirmen.

#### Estructura del repositorio

Esta es la estructura del repositorio git después de ejecutar `cdktf synth`. Los archivos `cdk.tf.json` contienen la configuración de Terraform
...
`--autoplan-file-list` para activar en archivos `cdk.tf.json` y `--include-git-untracked-files` establecido para incluir los
archivos Terraform generados dinámicamente por CDKTF en el plan de Atlantis.
1. Se hace push de la rama del PR que contiene cambios de código de `cdktf`.
...
**NOTA:** Si usa el archivo `atlantis.yaml` del repositorio, deberá especificar cada directorio que sea un proyecto Terragrunt.
...
Si no quiere crear/administrar usted mismo el archivo `atlantis.yaml` del repositorio, puede usar la herramienta [terragrunt-atlantis-config](https://github.com/transcend-io/terragrunt-atlantis-config) para generarlo.
...
Luego usamos archivos `.tfvars` para cargar diferentes variables para cada entorno.
...
Luego referenciaría los workflows en su `atlantis.yaml` de nivel de repositorio:
...
### Agregar contexto de directorio y repositorio para recursos aws usando tags predeterminados
...
Se pueden agregar otras variables predeterminadas, como para workspace. Vea a continuación más variables de entorno disponibles.
...
* Agregar tags a cada recurso puede regenerar data sources como `aws_iam_policy_document` lo que hará que muchos recursos se modifiquen. Vea el problema conocido en el proveedor aws [#29421](https://github.com/hashicorp/terraform-provider-aws/issues/29421).
...
| Key                             | Type   | Default | Required | Description                                                                                                                  |
|---------------------------------|--------|---------|----------|------------------------------------------------------------------------------------------------------------------------------|
| init/plan/apply/import/state_rm | string | none    | no       | Usar un comando integrado sin configuración adicional. Solo `init`, `plan`, `apply`, `import` y `state_rm` son compatibles |
...
| Key | Type | Default | Required | Description |
| --- | --- | --- | --- | --- |
| init/plan/apply/import/state_rm | map\[`extra_args` -> array\[string\]\] | none | no | Usar un comando integrado y agregar `extra_args`. Solo `init`, `plan`, `apply`, `import` e `state_rm` son compatibles como claves y solo `extra_args` es compatible como valor |
...
Ejemplo completo, filtrando salida y enmascarando el texto coincidente (`mySecret: "foo"` -> `mySecret: "<redacted>"`):
...
| run.shell | string | "sh" | no | Nombre del shell a usar para la ejecución del comando |
...
| run.output | string or []string or []any | "show" | no | Cómo postprocesar la salida de este comando cuando se publica en el comentario del PR. Las opciones son:<br/>*`show` - preservar la salida completa<br/>* `hide` - ocultar la salida del comentario (aún visible en la salida de streaming en tiempo real)<br/> `strip_refreshing` - ocultar toda la salida hasta e incluyendo la última línea que contiene "Refreshing...". Esto coincide con el comportamiento del comando integrado `plan` <br/> `filter_regex: "<regex_pattern>"` - enmascara texto sensible en comentarios de Atlantis reemplazando coincidencias de regex con &lt;redacted&gt;. Puede usarse múltiples veces (procesado en orden). Solo filtra comentarios inline - los enlaces completos al plan aún muestran resultados sin filtrar. |
...
* Los pasos `run` en el `workflow` principal se ejecutan con las siguientes variables de entorno:
  ...
  * `PLANFILE` - Ruta absoluta a la ubicación donde Atlantis espera que el plan
      ...
      Un workflow cuyos `plan` y `apply` estén compuestos enteramente de pasos personalizados `run`
      puede escribir su plan en una ruta elegida por sí mismo en lugar de `$PLANFILE`. Atlantis
      no requiere, no hace hash ni elimina un artefacto de plan para tal workflow; aun así
  ...
  * `PROJECT_NAME` - Nombre del proyecto configurado en `atlantis.yaml`. Si no se configura nombre de proyecto, esto será una cadena vacía.
  ...
  * `USER_NAME` - Nombre de usuario del usuario del VCS que ejecuta el comando, ej. `acme-user`. Durante un autoplan, el usuario será el usuario API de Atlantis, ej. `atlantis`.
  * `COMMENT_ARGS` - Cualquier flag adicional pasado en el comentario del pull request. Los flags se separan por comas y
      cada carácter se escapa, ej. `atlantis plan -- arg1 arg2` resultará en `COMMENT_ARGS=\a\r\g\1,\a\r\g\2`.
...
| Key | Type | Default | Required | Description |
| ----------------- | ----------------------- | --------- | ---------- | ----------------------------------------------------------------------------------------------------------------- |
| env | map\[string -> string\] | none | no | Establecer variables de entorno para pasos posteriores |
| env.name | string | none | yes | Nombre de la variable de entorno |
| env.value | string | none | no | Establecer el valor de la variable de entorno a una cadena codificada. No se puede establecer al mismo tiempo que `command` |
| env.command | string | none | no | Establecer el valor de la variable de entorno a la salida de un comando. No se puede establecer al mismo tiempo que `value` |
| env.shell | string | "sh" | no | Nombre del shell a usar para la ejecución del comando. No se puede establecer sin `command` |
...
| multienv.shell | string | "sh" | no | Nombre del shell a usar para la ejecución del comando |
...
| multienv.output | string | "show" | no | Establecer la salida en "hide" suprimirá el mensaje sobre las variables de entorno agregadas |
