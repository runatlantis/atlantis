...
executar `terraform plan` en los directorios que considera que contienen proyectos de Terraform modificados.

El algoritmo que usa es el siguiente:

1. Obtener la lista de todos los archivos modificados en el pull request
1. Filtrar a aquellos que coincidan con la [`--autoplan-file-list`](server-configuration.md#autoplan-file-list) configurada (por defecto incluye `.tf`, `.tf.json`, `.tfvars`, `.tofu`, `.tofu.json`, etc.)
...
contiene un archivo indicador de proyecto soportado (`*.tf`, `*.tf.json`, `*.tofu`, `*.tofu.json`, o `terragrunt.hcl`) ejecutar plan en ese directorio, de lo contrario ignorar el cambio (ver abajo para las excepciones).
...
* Si se modificara `modules/module1/main.tf`, no ejecutaríamos automáticamente `plan` porque no podríamos determinar la ubicación del proyecto de Terraform
  * Podrías usar un archivo [atlantis.yaml](repo-level-atlantis-yaml.md#configuring-planning) para especificar qué proyectos hacer plan cuando este módulo cambie
  * Podrías habilitar [module autoplanning](server-configuration.md#autoplan-modules), que indexa proyectos según sus dependencias de módulos locales.
  * O podrías hacer plan manualmente con `atlantis plan -d <dir>`
...
Bitbucket no tiene un webhook que se active solo cuando hay un PR nuevo o un commit nuevo. Para solucionar esto, almacenamos en caché el último commit para ver si ha cambiado. Si la caché se vacía, Atlantis pensará que tu commit es nuevo y podrías ver planes adicionales.
...
* [Deshabilitar Autoplanning](repo-level-atlantis-yaml.md#disabling-autoplanning)
