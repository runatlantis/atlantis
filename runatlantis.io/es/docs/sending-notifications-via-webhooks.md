...
Puede haber muchos webhooks: enviando notificaciones a diferentes destinos o para diferentes
...
Si está desplegando Atlantis como un chart de Helm, esto puede implementarse mediante el parámetro `config` disponible para [personalizaciones del chart](https://github.com/runatlantis/helm-charts#customization):
...
(resultado de un parámetro no establecido) coincide con cualquier cadena.
...
La información del evento `apply` será enviada por POST a `https://example.com/hooks`.
...
* Desplácese hacia abajo hasta Scopes | Bot Token Scopes y agregue los siguientes scopes de OAuth:
  * `channels:read`
  * `chat:write`
  * `groups:read`
  * `incoming-webhook`
...
* Agregue la app al canal creado o a un canal existente (haga clic en el nombre del canal, luego en la pestaña integrations, allí haga clic en "Add apps"
...
::: tip NOTE
A diferencia de los webhooks de apply, los webhooks de drift no admiten filtrado `workspace-regex` ni `branch-regex` porque la detección de drift opera a nivel de repositorio, no en el contexto de un pull request.
:::
...
* **Texto**: "Drift detected in owner/repo" o "No drift in owner/repo"
* **Campos**: Repository, Ref, Projects with drift (count), Detection ID
...
Los mismos headers `--webhook-http-headers` configurados para webhooks de apply también se envían con las solicitudes del webhook de drift.
