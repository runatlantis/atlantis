...
Atlantis necesita recibir Webhooks de tu host de Git para que pueda responder a eventos de pull request.
...
* Has [deployed](deployment.md) Atlantis y tienes una url para este
:::
...
puedes querer instalarlo solo en repos específicos por ahora.
:::

Al autenticarse como una GitHub App, los Webhooks se crean automáticamente y no necesitan configuración adicional, más allá de estar instalados en tu cuenta de organization/usuario después de la creación. Consulta la sección [GitHub App setup](access-credentials.md#github-app) para ver instrucciones sobre cómo hacerlo.
...
* establece **Secret** en el Webhook Secret que generaste previamente
...
* verifica de nuevo que agregaste `/events` al final de tu URL.
* establece **Secret Token** en el Webhook Secret que generaste previamente
  * **NOTE** Si estás agregando un webhook a múltiples repositorios, cada repositorio necesitará usar el **mismo** secret.
* marca las casillas
  * **Push events**
  * **Comments**
...
* Haz clic en **Next**
* Establece **URL** en `http://$URL/events` donde `$URL` es donde Atlantis está alojado. Ten en cuenta que SSL, o `https://$URL/events`, es requerido si estableces un nombre de usuario y contraseña Basic para el webhook. **Asegúrate de agregar `/events`**
* Se recomienda firmemente establecer un nombre de usuario y contraseña Basic para todos los webhooks
* Deja los tres menús desplegables para `...to send` establecidos en **All**
* Resource version debe establecerse en **1.0** para los tipos de evento `Pull request created` e `Pull request updated` y en **2.0** para `Pull request commented on`
...
Repite el proceso anterior hasta que tengas suscripciones de webhook para los siguientes tipos de evento que se activarán en todos los repositorios que Atlantis gestionará:
