...
Atlantis requiere que se cumplan ciertas condiciones **antes** de que los comandos `atlantis apply` e `atlantis import` puedan ejecutarse:

* [Approved](#approved) – requiere que los pull requests sean aprobados por al menos un usuario distinto del autor
* [Mergeable](#mergeable) – requiere que los pull requests puedan fusionarse
...
:::tip Consejo
Para requerir que **ciertas personas** aprueben el pull request, consulte el requisito [mergeable](#mergeable).
:::

### Mergeable

El requisito `mergeable` impedirá los applies a menos que un pull request pueda fusionarse.
...
::: warning
Algunos proveedores de VCS tienen una función de protección de ramas para controlar la "mergeability". Para usarla, limite la rama base para no omitir la protección de ramas.
Vea también la palabra clave `branch` en [Server Side Repo Config](server-side-repo-config.md#reference) para más detalles.
:::

#### GitHub

En GitHub, si no está usando [Protected Branches](https://docs.github.com/en/repositories/configuring-branches-and-merges-in-your-repository/defining-the-mergeability-of-pull-requests/about-protected-branches), entonces todos los pull requests son mergeable a menos que haya un conflicto.

Si configura Protected Branches, entonces puede hacer cumplir:
...
Consulte [GitHub: About protected branches](https://docs.github.com/en/repositories/configuring-branches-and-merges-in-your-repository/defining-the-mergeability-of-pull-requests/about-protected-branches) para más detalles.

::: warning
Si tiene el requisito **Restrict who can push to this branch**, entonces el usuario de Atlantis debe ser parte de esa lista para que considere un pull request como mergeable.
:::

::: warning
Si establece `atlantis/apply` en el requisito mergeable, use el flag `--gh-allow-mergeable-bypass-apply` o establezca la variable de entorno `ATLANTIS_GH_ALLOW_MERGEABLE_BYPASS_APPLY=true`. Este flag y esta variable de entorno permiten que la verificación de mergeable antes de ejecutar `atlantis apply` omita comprobar el estado de `atlantis/apply`.
:::

#### GitLab

Para GitLab, una merge request se fusionará si todo lo siguiente es verdadero:
...
* No está por detrás de la rama en la que se está fusionando, si los [Merge Methods](https://docs.gitlab.com/user/project/merge_requests/methods/) del proyecto son "Fast-forward merge" o "Merge commit with semi-linear history"

Para pipelines, si el proyecto requiere que los pipelines se completen correctamente, se comprobarán todas las builds excepto el estado del comando apply.

Para Jobs con la configuración allow_failure establecida en true, se ignorarán. Si el pipeline ha sido omitido y el proyecto permite fusionar, se marcará como mergeable.

Para comandos `atlantis apply` por proyecto, Atlantis limita el requisito `mergeable` de GitLab al proyecto que se está aplicando cuando GitLab informa estados de commit bloqueantes. Atlantis ignora estados de plan bloqueantes de Atlantis de otros proyectos en la misma merge request, pero el propio estado de plan del proyecto y cualquier otro bloqueador que no sea de Atlantis siguen impidiendo el apply.

#### Bitbucket.org (Bitbucket Cloud) y Bitbucket Server (Stash)

Para Bitbucket, solo comprobamos si hay un conflicto que impide una fusión. No comprobamos nada más porque la API de Bitbucket no lo soporta.

Si necesita una comprobación específica, por favor [open an issue](https://github.com/runatlantis/atlantis/issues/new).

#### Azure DevOps

En Azure DevOps, todos los pull requests son mergeable a menos que haya un conflicto. Puede establecer un pull request en "Complete" de inmediato, o establecer "Auto-Complete", que fusionará después de que se cumplan todas las políticas de rama. Consulte [Review code with pull requests](https://docs.microsoft.com/en-us/azure/devops/repos/git/pull-requests?view=azure-devops).
...
* Permitir a los usuarios aprobar sus propios cambios
...
La estrategia de checkout `merge` crea un commit de fusión temporal y ejecuta `plan` sobre la versión local en Atlantis de las ramas de origen y destino del PR.
La rama de destino local puede quedar desactualizada ya que los cambios a la rama de destino no se obtienen si no hay cambios en la rama de origen. `undiverged` hace cumplir que la versión local de main en Atlantis esté actualizada con la remota para que el estado del origen durante el `apply` sea idéntico al que tendría si fusionara el PR en ese momento. En el caso de un error transitorio, Atlantis asume divergencia por seguridad y produce un error.

Cuando un proyecto tiene configurados patrones `autoplan.when_modified`, el requisito `undiverged` usa automáticamente esos patrones para realizar una comprobación de divergencia **dirigida**. En lugar de fallar cuando **cualquier** archivo en la rama base ha cambiado, solo falla cuando han cambiado archivos que coinciden con los patrones `when_modified` del proyecto. Esto es especialmente útil en monorepos donde cambios no relacionados en otros proyectos no deberían bloquear sus applies.

Las comprobaciones `undiverged` dirigidas también siguen la selección de proyectos de Atlantis para:
...
## Establecer requisitos de comando

Como se mencionó anteriormente, puede establecer requisitos de comando mediante flags, en `repos.yaml`, o en `atlantis.yaml` si `repos.yaml` permite la sobrescritura.

### Los flags tienen prioridad

Los flags **sobrescriben** cualquier configuración `repos.yaml` o `atlantis.yaml`, por lo que equivalen a tener siempre establecido ese requisito de apply.
...
1. Especificar qué proyectos tienen qué requisitos mediante un archivo `atlantis.yaml`, y permitir que `plan_requirements`, `apply_requirements` y `import_requirements` se establezcan en `atlantis.yaml` mediante la configuración `repos.yaml` del lado del servidor.


   Por ejemplo, si tengo dos directorios, `staging` e `production`, podría usar:
...
Una vez que se cumple el requisito de apply, **cualquiera** que pueda comentar en el pull request puede ejecutar el comando `atlantis apply` real.
