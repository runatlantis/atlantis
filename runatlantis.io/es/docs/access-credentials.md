...
Una vez que haya creado un nuevo usuario (o decidido usar uno existente), necesita
generar un token de acceso. Continúe leyendo las instrucciones para su host Git específico:
...
Los webhooks pueden crearse manualmente o ser administrados por la GitHub App para los repositorios que activan Atlantis. Si los crea manualmente (vea la [sección siguiente](access-credentials.md#manually-creating-the-github-app)), no especifique detalles del webhook en la configuración de la GitHub app. En ambos casos se recomienda encarecidamente proteger los webhooks usando un secreto. Vea [Webhook Secrets](webhook-secrets.md#webhook-secrets)
:::
...
Una vez que tenga su usuario y token de acceso, estará listo para crear un secreto de webhook. Vea [Creating a Webhook Secret](webhook-secrets.md).
