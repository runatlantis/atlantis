...
- Detectar eliminaciones de recursos no intencionales
- Prevenir riesgos de seguridad (es decir, exponer puertos seguros al público)

## ¿Cómo funciona?

Habilitar "policy checking" además del [requisito de apply mergeable](command-requirements.md#supported-requirements) bloquea los apply en planes que fallen cualquiera de las políticas de conftest definidas.
...
Cualquier fallo necesita ser corregido ya sea en un commit posterior, o aprobado por los owner(s) de nivel superior de las políticas o los owner(s) del conjunto de políticas en cuestión. Las aprobaciones de políticas son independientes del requisito de apply con aprobación, que puede coexistir en el workflow de policy checking. Después de que las políticas sean aprobadas, el apply puede continuar.
...
::: warning
Por defecto, cualquier plan posterior a la aprobación descartará todas las aprobaciones de políticas y las solicitará de nuevo. Para cambiar este comportamiento, consulte [Sticky Policy Approvals](#sticky-policy-approvals).
:::

## Primeros pasos

Esta sección proporcionará una guía sobre cómo configurarse con una política simple que falle la creación de `null_resource` y requiera aprobación de un usuario autorizado.

### Paso 1: Habilitar el workflow

Habilite el workflow usando la siguiente flag de configuración del servidor `--enable-policy-checks`
...
::: warning NOTE
Si está usando la flag [`--gh-team-allowlist`](server-configuration.md#gh-team-allowlist) para restringir qué equipos pueden ejecutar comandos, **debe** también incluir en la allowlist el comando `policy_check` para que las verificaciones de políticas funcionen en comandos `atlantis plan` manuales.
...
Alternativamente, puede usar `allowed_overrides: [policy_check]` en su [configuración de repositorio del lado del servidor](server-side-repo-config.md).
...
- Sin incluir `policy_check` en la allowlist, los comandos `atlantis plan` manuales planificarán correctamente pero omitirán las verificaciones de políticas

Consulte [Repo and Project Permissions](repo-and-project-permissions.md#server-option-gh-team-allowlist) para más información sobre las allowlists de equipos.
:::

### Paso 2: Definir la configuración de políticas

La Policy Configuration se define en la [configuración de repositorio del lado del servidor](server-side-repo-config.md#reference).
...
- `source` - Indica a atlantis desde dónde obtener las políticas. Actualmente solo puede alojar políticas localmente usando `local`.
- `owners` - Define los usuarios/equipos que pueden aprobar un conjunto de políticas específico.
- `approve_count` - Define el número de aprobaciones necesarias para omitir las verificaciones de políticas. Por defecto toma el valor de la configuración de políticas de nivel superior, si no se especifica.
...
::: tip Note
Conftest se ejecuta con un vector de argumentos en lugar de a través de un shell. Un conjunto de
políticas `path` y cada entrada `extra_args` todavía se dividen por espacios en blanco, por lo que un valor
que contiene más de un argumento, como `"-p /home/atlantis/conftest_policies/"`
de arriba, sigue funcionando. Los operadores de shell y la sustitución de comandos se pasan
como texto ordinario. Para usar una ruta que realmente contenga un espacio, póngala entre comillas,
por ejemplo
`"'/home/atlantis/my policies'"`. Las referencias de entorno en texto sin comillas o
entre comillas dobles se expanden desde el entorno de policy-check sin
dividir el valor expandido. Use comillas simples o una barra invertida antes de `$` cuando
la referencia deba permanecer literal.
:::

### Paso 3: Escribir la política

Las políticas de Conftest se basan en [Open Policy Agent (OPA)](https://www.openpolicyagent.org/) y están escritas en [rego](https://www.openpolicyagent.org/docs/latest/policy-language/#what-is-rego). Siguiendo nuestro ejemplo, simplemente cree un archivo `rego` en la carpeta `null_resource_warning` con el siguiente código; el código de abajo es una política simple que fallará para planes que contengan `null_resource` recién creados.
...
Por defecto, cuando un plan se vuelve a ejecutar, todas las aprobaciones de políticas previas se descartan. Esto significa que después de cada `atlantis plan`, los owner de políticas deben volver a aprobar incluso si nada de los fallos de políticas cambió.

Las **sticky policy approvals** permiten que las aprobaciones sobrevivan a los re-plan, siempre que no aparezcan elementos nuevos en la salida de políticas (coincidentes mediante `policy_item_regex`). Esto es útil en workflows donde los planes se vuelven a ejecutar con frecuencia (por ejemplo, debido a actualizaciones de la rama base) pero las violaciones de políticas siguen siendo las mismas o están siendo resueltas.

### Cómo funciona

Cuando las aprobaciones persistentes están habilitadas, Atlantis extrae elementos de la salida de políticas usando `policy_item_regex` y les aplica hash. Cada aprobación registra una instantánea de estos hashes. En un re-plan, las aprobaciones se mantienen solo si sus hashes registrados todavía cubren la salida actual. Agregar o cambiar elementos invalida las aprobaciones; eliminar elementos (por ejemplo, corregir una violación) las preserva.
...
### Personalizar la regex de elementos de la política

`policy_item_regex` controla qué partes de la salida de políticas se usan como la "identidad" para el seguimiento de aprobaciones persistentes. El valor predeterminado es `(?s).+`, que hace coincidir la salida completa como un único elemento. Esto significa que cualquier cambio en la salida invalida la aprobación: un enfoque de todo o nada.

Para un seguimiento granular por elemento, sobrescriba la regex para que coincida con elementos individuales. Por ejemplo, `.+` (sin `(?s)`) hace coincidir cada línea no vacía como un elemento separado:
...
Con `.+`, cada línea se rastrea independientemente. Las aprobaciones siguen siendo válidas siempre que cada línea actual estuviera presente en el momento de la aprobación. Corregir una violación (eliminar una línea) preserva las aprobaciones existentes; agregar o cambiar una las invalida.

::: tip
La semántica de subconjunto de arriba es intencional: una aprobación cubre un conjunto específico de elementos, y cualquier elemento actual debe haber sido parte de ese conjunto. Eliminar elementos (por ejemplo, un autor de políticas corrige una violación previamente marcada) se trata como progreso y **no** invalida aprobaciones previas. Solo la aparición de elementos nuevos o cambiados desencadena una nueva aprobación.
:::

Para salida de texto, use `(?m)^FAIL.*` para rastrear solo líneas que comiencen con `FAIL`:
...
::: tip
Use el prefijo de flag `(?m)` para habilitar coincidencia multilínea, de modo que `^` y `$` coincidan con el inicio y el final de cada línea en lugar de solo el inicio y el final de toda la cadena.
:::

Una `policy_item_regex` por conjunto de políticas puede sobrescribir el valor predeterminado de nivel superior:
...
Cuando las aprobaciones persistentes se usan con `approve_count > 1`, el mismo usuario no puede proporcionar múltiples aprobaciones para el mismo conjunto de políticas con los mismos hashes. Si un usuario ya ha aprobado completamente un conjunto de políticas y los elementos extraídos no han cambiado, intentos posteriores de aprobación por el mismo usuario producirán un error solicitando que un owner de políticas diferente apruebe.

## Ejecutar la verificación de políticas solo en algunos repositorios

Cuando policy checking está habilitado, se aplicará en todos los repositorios; para deshabilitar policy checking en algunos repositorios, primero [habilite policy checks](policy-checking.md#getting-started) y luego desactívelo explícitamente en cada repositorio con la flag `policy_check`.

Para la configuración del lado del servidor:
...
Para la configuración `atlantis.yaml` a nivel de repositorio:
