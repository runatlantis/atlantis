...
Las métricas se configuran mediante la [configuración del lado del servidor](server-side-repo-config.md#metrics).

## Métricas disponibles

Suponiendo que las métricas se exponen desde el endpoint `/metrics` de la configuración del lado del servidor de [metrics](server-side-repo-config.md#metrics), por ejemplo.
...
Para ver todas las métricas expuestas por el servicio atlantis, haga una solicitud GET al endpoint `/metrics`.
...
|------------------------------------------------|----------------------------------------------------------------------|-------------------------------------------------------------------------------------|
| `atlantis_cmd_autoplan_execution_error`        | [counter](https://prometheus.io/docs/concepts/metric_types/#counter) | número de veces que [autoplan](autoplanning.md#autoplanning) ha generado un error.     |
| `atlantis_cmd_comment_plan_execution_error`    | [counter](https://prometheus.io/docs/concepts/metric_types/#counter) | número de veces que al comentar `atlantis plan` ha generado un error.                |
...
| `atlantis_cmd_comment_apply_execution_error`   | [counter](https://prometheus.io/docs/concepts/metric_types/#counter) | número de veces que al comentar `atlantis apply` ha generado un error.               |
