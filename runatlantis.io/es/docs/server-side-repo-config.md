# Config de repositorio del lado del servidor

Se usa un archivo de Server-Side Config para más grupos de configuración del servidor que no pueden expresarse razonablemente mediante flags.

Un caso de uso de este tipo es controlar el comportamiento por repositorio
y lo que los usuarios pueden hacer en archivos de nivel de repositorio `atlantis.yaml`.

## ¿Necesito un archivo de config del lado del servidor?

No necesita un archivo de config de repositorio del lado del servidor a menos que quiera personalizar
...
Para usar config de repositorio del lado del servidor, cree un archivo de config, por ej. `repos.yaml`, y páselo al
comando `atlantis server` mediante la flag `--repo-config`, por ej. `--repo-config=path/to/repos.yaml`.

Si no desea escribir un archivo de config en disco, puede usar la
flag `--repo-config-json` o la variable de entorno `ATLANTIS_REPO_CONFIG_JSON`
para especificar su config como JSON. Consulte [--repo-config-json](server-configuration.md#repo-config-json)
para ver un ejemplo.
...
Aquí hay algunas de las razones por las que podría querer usar una config de repositorio.
...
Consulte [Command Requirements](command-requirements.md) para más detalles.
...
Para permitir que todos los repositorios sobrescriban el predeterminado:
...
Para permitir que solo un repositorio específico sobrescriba el predeterminado:
...
establezca `plan_requirements`, `apply_requirements` o `import_requirements` como un arreglo vacío (deshabilitando el requisito).
...
Consulte [Pre Workflow Hooks](pre-workflow-hooks.md) para más detalles sobre cómo escribir
...
Consulte [Post Workflow Hooks](post-workflow-hooks.md) para más detalles sobre cómo escribir
hooks de post workflow.

### Cambiar el workflow predeterminado de Atlantis

Si quiere cambiar los comandos predeterminados que Atlantis ejecuta durante las fases `plan` y `apply`,
...
Consulte [Custom Workflows](custom-workflows.md) para más detalles sobre cómo escribir
workflows personalizados.

### Permitir que los repositorios elijan un workflow del lado del servidor

Si quiere que los repositorios puedan elegir sus propios workflows definidos
en la config de repositorio del lado del servidor, necesita crear los workflows
...
### Permitir usar herramientas de política personalizadas

Conftest es la aplicación estándar de verificación de políticas integrada con Atlantis, pero todavía se pueden ejecutar herramientas personalizadas en workflows personalizados cuando la opción `custom_policy_check` está establecida. Consulte la [página de Custom Policy Checks](custom-policy-checks.md) para ejemplos detallados.
...
request a ese repositorio puede esencialmente ejecutar código arbitrario en su servidor Atlantis.
:::
...
Ejecutar múltiples servidores Atlantis para manejar el mismo repositorio se puede hacer para separar permisos para cada servidor Atlantis.
En este caso, se puede usar un archivo de config de repositorio [atlantis.yaml](repo-level-atlantis-yaml.md) diferente usando diferentes archivos `repos.yaml`.

Por ejemplo, considere una situación en la que Atlantis de `production-server` separado usa la config de repositorio `atlantis-production.yaml` y Atlantis de `staging-server` usa la config de repositorio `atlantis-staging.yaml`.
...
El archivo `repos.yaml` contiene la clave `repo_config_file` para especificar la ruta del archivo de config atlantis del repositorio.
...
Consulte los ejemplos de configuración en [atlantis.yaml](repo-level-atlantis-yaml.md).
...
Ahora, se pueden configurar 2 URL de webhook para el repositorio, que envían eventos a `production-server` e `staging-server` respectivamente.
Cada servidor maneja diferentes archivos de config del repositorio.

:::tip Notes

* Si los comentarios `no projects` son molestos, configure [--silence-no-projects](server-configuration.md#silence-no-projects).
* El nombre del ejecutable disparador de comandos puede reconfigurarse de `atlantis` a otra cosa estableciendo [Executable Name](server-configuration.md#executable-name).
* Cuando use diferentes usuarios vcs del servidor atlantis como `@atlantis-staging`, el comentario `@atlantis-staging plan` puede usarse en lugar de `atlantis plan` para llamar solo a `staging-server`.
:::
...
| workflows  | map[string: [Workflow](custom-workflows.md#workflow)] | see below | no       | Mapa de nombre de workflow a workflow. Los workflows sobrescriben los comandos predeterminados de Atlantis. |
...
| Key | Type | Default | Required | Description |
| --- | --- | --- | --- | --- |
| id | string | none | yes | El valor puede ser una expresión regular cuando se especifica como /&lt;regex&gt;/ o una coincidencia exacta de cadena. Los ID de repositorio son de la forma `{vcs hostname}/{org}/{name}`, por ej. `github.com/owner/repo`. El hostname se especifica sin esquema ni puerto. Para Bitbucket Server, {org} es el **nombre** del proyecto, no la clave. |
| branch | string | none | no | Una regex que hace coincidir pull requests por rama base (la rama en la que se fusiona el pull request). De forma predeterminada, todas las ramas coinciden |
| repo_config_file | string | none | no | Ruta del archivo de config del repositorio en este repositorio. De forma predeterminada, use `atlantis.yaml`, que se encuentra en la raíz del repositorio. Cuando múltiples servidores atlantis trabajan con el mismo repositorio, configure nombres de archivo diferentes. |
| workflow | string | none | no | Un workflow personalizado. |
| plan_requirements | []string | none | no | Requisitos que deben satisfacerse antes de que pueda ejecutarse `atlantis plan`. Actualmente los únicos requisitos compatibles son `approved`, `mergeable` y `undiverged`. Consulte [Command Requirements](command-requirements.md) para más detalles. |
| apply_requirements | []string | none | no | Requisitos que deben satisfacerse antes de que pueda ejecutarse `atlantis apply`. Actualmente los únicos requisitos compatibles son `approved`, `mergeable` y `undiverged`. Consulte [Command Requirements](command-requirements.md) para más detalles. |
| import_requirements | []string | none | no | Requisitos que deben satisfacerse antes de que pueda ejecutarse `atlantis import`. Actualmente los únicos requisitos compatibles son `approved`, `mergeable` y `undiverged`. Consulte [Command Requirements](command-requirements.md) para más detalles. |
...
| repo_locking | bool | false | no | (deprecated) Si se obtiene o no un lock. |
| repo_locks | [RepoLocks](#repolocks) | `mode: on_plan` | no | Si los locks del repositorio están habilitados o no para este proyecto en plan o apply. Consulte [RepoLocks](#repolocks) para más detalles. |
...
| autodiscover | AutoDiscover | none | no | Configuración de autodiscover para este repositorio |
| silence_pr_comments | []string | none | no | Silencia los comentarios del PR de etapas definidas mientras preserva las verificaciones de estado del PR. Útil en entornos grandes con muchas instancias de Atlantis y/o proyectos, cuando los comentarios son demasiado grandes y demasiados, por lo tanto es preferible confiar únicamente en las verificaciones de estado del PR. Los valores compatibles son: `plan`, `apply`. |

:::tip Notes

* Si coinciden múltiples repositorios, se aplicará la última coincidencia.
* Si una clave no está definida, no sobrescribirá una clave que haya coincidido arriba.
  Por ejemplo, dado un ID de repositorio `github.com/owner/repo` y una config:
  ...
  * `apply_requirements` se establece desde la config `id: github.com/owner/repo` porque
    sobrescribe la config coincidente anterior de `id: /.*/`.
...
| approve_count | int | 1 | no | cantidad de aprobaciones requeridas para omitir políticas fallidas. |
| sticky_policy_approvals | bool | false | no | cuando es true, las aprobaciones de políticas sobreviven a re-plans siempre que no se introduzcan nuevos elementos de salida de política (según `policy_item_regex`). Consulte [Sticky Policy Approvals](policy-checking.md#sticky-policy-approvals). |
| policy_item_regex | string | `(?s).+` | no | regex para extraer elementos comparables de la salida de política para el seguimiento de aprobación sticky. El valor predeterminado hace coincidir la salida completa como un solo elemento. Consulte [Sticky Policy Approvals](policy-checking.md#sticky-policy-approvals). |
...
| source                   | string | none      | yes      | por el momento solo se admite `local`                                                                                                                    |
...
| approve_count            | int    | inherited | no       | cantidad de aprobaciones requeridas. El valor predeterminado es el valor `approve_count` de nivel superior                                                                             |
...
| sticky_policy_approvals  | bool   | inherited | no       | sobrescribe el `sticky_policy_approvals` de nivel superior para este conjunto de políticas. Consulte [Sticky Policy Approvals](policy-checking.md#sticky-policy-approvals).         |
| policy_item_regex        | string | inherited | no       | sobrescribe el `policy_item_regex` de nivel superior para este conjunto de políticas. Consulte [Sticky Policy Approvals](policy-checking.md#sticky-policy-approvals).               |
