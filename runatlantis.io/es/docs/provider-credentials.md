...
Atlantis ejecuta Terraform simplemente ejecutando los comandos `terraform plan` e `apply`
en el servidor donde Atlantis está alojado.
...
* The Atlantis [Helm Chart](deployment.md#kubernetes-helm-chart) y
    [AWS Fargate Module](deployment.md#aws-fargate) tienen sus propios mecanismos para las credenciales del proveedor. Lee su documentación.
* Si estás ejecutando Atlantis en una cloud, entonces muchas clouds tienen formas de dar acceso a la API de cloud
  a las aplicaciones que se ejecutan en ellas, por ej.:
  * [AWS EC2 Roles](https://registry.terraform.io/providers/hashicorp/aws/latest/docs) (Busca "EC2 Role")
  * [GCE Instance Service Accounts](https://registry.terraform.io/providers/hashicorp/google/latest/docs/guides/provider_reference)
* Muchos usuarios establecen variables de entorno, por ej. `AWS_ACCESS_KEY`, donde Atlantis se está ejecutando.
* Otros crean los archivos de configuración necesarios, por ej. `~/.aws/credentials`, donde Atlantis se está ejecutando.
...
executando y ejecutar comandos `terraform` como lo harías localmente, entonces Atlantis funcionará.
:::
...
Atlantis soporta múltiples cuentas de AWS mediante el uso de
...
debes asegurarte de que el servidor en el que Atlantis se está ejecutando tenga el archivo de credenciales correspondiente.

Si estás usando [Assume role](https://registry.terraform.io/providers/hashicorp/aws/latest/docs) (Busca "Assume role")
debes asegurarte de que el archivo de credenciales tenga un perfil `default` que sea capaz
...
Si estás usando Terraform < 0.12, Atlantis inyecta 5 variables de Terraform que pueden usarse para nombrar dinámicamente el nombre de sesión de assume role.
Establecer `session_name` te permite rastrear llamadas a la API hechas a través de Atlantis hasta un
...
asegúrate de añadir la opción `role_arn`:
...
En Terraform >= 0.12, no se te permite establecer ningún flag `-var` si esas variables
no se están usando. Como no podemos saber si estás usando estas variables `atlantis_*`,
no podemos establecer el flag `-var`.
