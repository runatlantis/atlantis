...
La configuración para `atlantis server` puede especificarse mediante flags de línea de comandos,
...
El archivo de configuración de `--config` solo se usa como una forma alternativa de establecer flags de `atlantis server`.
:::
...
- Acepta una lista separada por comas, p. ej. `command1,command2`.
- `version`, `plan`, `apply`, `unlock`, `approve_policies`, `cancel`, `import`, `state`, `policy_check` y `all` están disponibles.
- `policy_check` es un comando interno que se ejecuta automáticamente después de `plan` cuando [policy checking](policy-checking.md) está habilitado. Debe incluirse explícitamente en la allowlist al usar [`--gh-team-allowlist`](#gh-team-allowlist).
...
ejecute código arbitrario. Esto puede suceder porque
Atlantis ejecutará automáticamente `terraform plan`
que puede ejecutar código arbitrario si se le da una configuración Terraform maliciosa.
:::
...
Secreto requerido usado para validar solicitudes realizadas a los [endpoints `/api/*`](api-endpoints.md).
...
donde `$port` proviene del flag [`--port`](#port). Soporta un basepath si aloja Atlantis bajo una ruta.
...
- Esta URL se usa como el enlace `details` junto a cada trabajo de atlantis para ver los logs del trabajo.
...
Establece el modo de auto discover, el valor predeterminado es `auto`. Cuando se establece en `auto`, los proyectos en un repo serán descubiertos por
Atlantis cuando no haya proyectos configurados en la configuración del repo. Si uno o más proyectos están definidos
en la configuración del repo entonces el auto discovery se deshabilitará completamente.
...
definido en la sección de proyectos de la configuración del repo, el proyecto de la configuración del repo tendrá precedencia sobre
...
Por defecto es `false`. Consulte [Automerging](automerging.md) para más detalles.
...
Método de fusión predeterminado para usar al hacer automerging de pull requests. Los valores válidos son
`merge`, `rebase` y `squash`. Cuando no se establece, se usa el método de fusión predeterminado
del proveedor VCS. Esto puede sobrescribirse por comando con el flag de comentario `--auto-merge-method`.
...
Lista de patrones de archivos que Atlantis usará para verificar si un directorio contiene archivos modificados que deben disparar el project planning.

Notas:

- Acepta una lista separada por comas, p. ej. `pattern1,pattern2`.
- Los patrones usan la [sintaxis de `.dockerignore`](https://docs.docker.com/engine/reference/builder/#dockerignore-file)
- La lista de patrones de archivos será usada tanto por los plans automáticos como por los ejecutados manualmente.
- Cuando no se establece, por defecto incluye todos los archivos `.tf`, `.tf.json`, `.tfvars`, `.tfvars.json`, `.tofu`, `.tofu.json`, `terragrunt.hcl` y `.terraform.lock.hcl`
   (`--autoplan-file-list='**/*.tf,**/*.tf.json,**/*.tfvars,**/*.tfvars.json,**/*.tofu,**/*.tofu.json,**/terragrunt.hcl,**/.terraform.lock.hcl'`).
- Establecer `--autoplan-file-list` sobrescribirá los valores predeterminados. **Debe** agregar `**/*.tf` y otros predeterminados si quiere incluirlos.
- El valor predeterminado es global (no consciente de la distribución). Tanto las instalaciones de Terraform como de OpenTofu coincidirán con los cambios de `.tofu`. Si no usa OpenTofu, puede sobrescribir el valor predeterminado para excluir patrones `.tofu`.
...
- Autoplan cuando cualquier archivo `*.tf` o `*.tfvars` se modifica.
  - `--autoplan-file-list='**/*.tf,**/*.tfvars'`
- Autoplan cuando cualquier archivo `*.tf` se modifica excepto en el directorio `project2/`
  - `--autoplan-file-list='**/*.tf,!project2'`
- Autoplan cuando cualquier archivo `*.tf` o archivos `.yml` en una subcarpeta de `project1` se modifica.
  - `--autoplan-file-list='**/*.tf,project1/**/*.yml'`

::: warning NOTE
De forma predeterminada, los cambios en módulos no dispararán autoplanning. Vea los flags a continuación.
:::
...
Después del rastreo, Atlantis hará plan de cualquier proyecto que incluya un módulo modificado. Esto es equivalente a establecer
`--autoplan-modules-from-projects` con el valor de `--autoplan-file-list`. Vea abajo.

::: tip NOTE
La indexación de dependencias de módulos usa inspección de configuración de Terraform y puede no soportar completamente archivos `.tofu` / `.tofu.json`. Las llamadas a módulos definidas solo en archivos `.tofu` o directorios de módulos compartidos que contengan solo archivos `.tofu` pueden no ser rastreadas. El autoplanning directo por cambio de archivo para proyectos `.tofu` funciona independientemente. Use patrones explícitos `autoplan.when_modified` como solución alternativa. Consulte [OpenTofu .tofu file support](terraform-versions.md#opentofu-tofu-file-support) para más detalles.
:::
...
Habilita auto-planing de proyectos cuando una dependencia de módulo en el mismo repositorio ha cambiado.
Esto es una lista de patrones de archivos como `autoplan-file-list`.
...
incluyendo proyectos que incluyen el módulo vía otros módulos. Cuando un archivo de módulo que coincide con `autoplan-file-list` cambia,
...
- `**/*.tf` - indexará todos los proyectos que tengan un archivo `.tf` en su directorio, y hará plan de ellos cada vez que una dependencia de módulo dentro del repo haya cambiado.
- `**/*.tf,!foo,!bar` - indexará todos los proyectos que contengan `.tf` excepto `foo` y `bar` y hará plan de ellos cada vez que una dependencia de módulo dentro del repo haya cambiado.
   Esto permite a los proyectos excluirse del auto-planning cuando cambia una dependencia de módulo.
...
flag permite seleccionar los _proyectos_ a indexar, pero el disparador para un plan debe ser un archivo en `autoplan-file-list`.
:::
...
Hostname de Azure DevOps para soportar instancias cloud y self-hosted. Por defecto es `dev.azure.com`.

::: warning COMPATIBILITY WARNING
Si está afectado por este cambio [docs](https://learn.microsoft.com/en-us/azure/devops/release-notes/2018/sep-10-azure-devops-launch#administration)
...
Ejemplos:
`https://dev.azure.com/MYCompany/` y `https://mycompany.visualstudio.com/` se convertirán en `mycompany`
`https://dev.azure.com/MYCOMPANY/` y `https://myCOMPANY.visualstudio.com/` se convertirán en `mycompany`
...
Ejecutar un atlantis unlock desde v0.35.0 en sus PR actuales ignorará los archivos en la carpeta `MYCompany`. En el siguiente atlantis plan se usará la carpeta `mycompany` y se generará todo en el nuevo nombre de carpeta
:::
...
atacante podría falsificar llamadas a Atlantis y hacer que realice acciones maliciosas.
Debe especificarse mediante la variable de entorno `ATLANTIS_AZUREDEVOPS_WEBHOOK_PASSWORD`.
:::
...
- La compatibilidad hacia atrás es para soportar las Bitbucket APP Passwords existentes que aún son válidas hasta junio de 2026 (vea [Atlassian's Bitbucket app password deprecation notice](https://www.atlassian.com/blog/bitbucket/bitbucket-cloud-transitions-to-api-tokens-enhancing-security-with-app-password-deprecation)).

**Clave de archivo de configuración:**
...
- Estos flags se bloquean para prevenir problemas de seguridad como traversal del directorio de trabajo (`-chdir`) o cargar providers maliciosos (`-plugin-dir`).
- Establecer este flag **reemplaza** completamente la lista predeterminada. Para extender los valores predeterminados, inclúyalos junto con sus flags personalizados, p. ej. `-chdir,--chdir,-plugin-dir,--plugin-dir,-my-flag`.
- Acepta una lista separada por comas, p. ej. `-flag1,-flag2`.
...
El número de commits a obtener de la rama. Se usa si `--checkout-strategy=merge` ya que la estrategia de checkout `--checkout-strategy=branch` (predeterminada) siempre usa por defecto un shallow clone con una profundidad de 1.
Por defecto es `0`. Consulte [Checkout Strategy](checkout-strategy.md) para más detalles.
...
Por defecto es `branch`. Consulte [Checkout Strategy](checkout-strategy.md) para más detalles.
...
Archivo de configuración YAML donde también pueden establecerse flags. Consulte [Config File](#config-file) para más detalles.
...
Por defecto es `~/.atlantis`. Atlantis almacenará aquí su base de datos, repos checkouteados, plans de Terraform por defecto y binarios de
...
Versión de Terraform a usar por defecto. Se descargará en `<data-dir>/bin/terraform<version>`
si no está en `PATH`. Consulte [Terraform Versions](terraform-versions.md) para más detalles.
...
Deshabilitar el comando `atlantis apply` para que deba
especificarse un proyecto/workspace/directorio específico para applies.
...
Por defecto es una cadena vacía, por lo que ninguna label deshabilita automerge por defecto.
Este flag no tiene efecto a menos que automerge esté habilitado con `--automerge` o
`automerge: true` a nivel de repo.
...
Deshabilitar atlantis auto planning.
...
Deshabilitar atlantis auto planning solo en pull requests con la label especificada.
...
Si es true, elimina el botón en la UI que permite a los usuarios deshabilitar globalmente los comandos apply.
...
Deshabilitar el plegado en la salida markdown usando la etiqueta html `<details>`.
...
Evita que atlantis bloquee proyectos y/o workspaces al ejecutar terraform.
...
Si se establece, descartar aprobación si se ha ejecutado un nuevo plan. Actualmente solo soportado en GitHub y GitLab. Para GitLab se requiere un bot, group o project token para esta feature.
...
Habilitar Atlantis para formatear la salida de Terraform plan en un formato amigable con markdown-diff para fines de coloración.

Útil para habilitar al usar GitHub. Las líneas cambiadas dentro de diffs de heredoc y multiline-string de Terraform también se formatean para que los renderizadores markdown conscientes de diff puedan colorearlas.
...
Habilitar endpoints de API de drift detection. Drift detection no ejecuta Terraform apply, pero
sí ejecuta el ciclo de vida normal de plan, incluyendo pre-workflow hooks configurados,
workflows personalizados, pasos de plan personalizados y comandos Terraform plan. Cuando está habilitado, Atlantis
inicializará almacenamiento en memoria para resultados de drift detection y un servicio de remediación,
haciendo funcionales los endpoints de drift detection, status y remediación solo-plan. Si [webhooks](sending-notifications-via-webhooks.md#drift-detection-webhooks) de drift
están configurados (`event: drift`), las ejecuciones de detección exitosas envían notificaciones a Slack o endpoints HTTP,
incluyendo resultados heartbeat sin drift. Drift detection no omite team allowlists ni checks de estado de PR
`plan_requirements` como `approved` o `mergeable`; esos checks fallan de forma cerrada cuando
...
rechazadas mientras que la drift detection de solo lectura permanece disponible. Este flag no omite
`apply_requirements` del repositorio; los requisitos que necesitan estado de pull request fallan de forma cerrada para
solicitudes de remediación no PR. Por defecto es `false`.
...
Habilitar backends de almacenamiento externo configurados en la repo config del lado del servidor (bloque `external_stores`). Cuando se establece, Atlantis lee la sección `external_stores` del YAML de configuración del repo para inicializar backends como S3 para persistencia de archivos de plan.
...
Habilita a atlantis para ejecutar policies del lado del servidor sobre el resultado de un terraform plan. Las policies están definidas en [server side repo config](server-side-repo-config.md#reference).
...
Habilitar endpoints [`net/http/pprof`](https://pkg.go.dev/net/http/pprof) para [continuous profiling](https://grafana.com/docs/pyroscope/latest/introduction/continuous-profiling/) de recursos usados por el servidor. Consulte [profiling Go programs](https://go.dev/blog/pprof) para más información.
...
Habilitar Atlantis para usar expresiones regulares para ejecutar comandos plan/apply contra nombres de proyecto definidos cuando se pasa con él el flag `-p`.

Esto puede usarse para ejecutar todos los proyectos definidos (con la clave `name`) en `atlantis.yaml` usando `atlantis plan -p .*`.

El flag solo permitirá las regex listadas en la clave [`allowed_regexp_prefixes`](repo-level-atlantis-yaml.md#reference) definida en el archivo repo `atlantis.yaml`. Si la clave no está definida, su valor por defecto es `[]` lo que permitirá cualquier regex.

Esto aún no funcionará con `-d` y para usar `-p` los proyectos del repo deben definirse en el archivo repo `atlantis.yaml`.

Cuando `--restrict-file-list` está habilitado, los plans de proyecto regex se limitan a coincidir con proyectos con archivos modificados en el pull request. Sin `--restrict-file-list`, los comandos de proyecto regex aún pueden ejecutarse contra todos los proyectos coincidentes.

::: warning SECURITY WARNING
No se supone que deba usarse con `--disable-apply-all`.
...
Fallar y no ejecutar el comando Atlantis solicitado si cualquiera de los pre workflow hooks da error.
...
ID de GitHub app. Si se establece, la autenticación de GitHub se realizará como [an installation](https://docs.github.com/en/rest/apps/installations).

::: tip
Un GitHub app puede crearse iniciando Atlantis primero, luego apuntando su navegador a
...
El installation ID de una instancia específica de una aplicación GitHub. Normalmente este valor se
deriva consultando GitHub para la lista de instalaciones del ID suministrado mediante `--gh-app-id` y seleccionando
la primera encontrada y donde múltiples instalaciones resultan en un error. Use este flag si tiene múltiples
instancias de Atlantis pero quiere usar una sola GitHub app ya instalada para todas ellas. Normalmente haría esto si
está ejecutando un proxy como su única aplicación GitHub que hará proxy a una instancia apropiada de Atlantis
basándose en la organización o usuario que disparó el webhook.
...
::: warning SECURITY WARNING
El contenido de la clave privada será visible para cualquiera que pueda ejecutar `ps` o mirar el historial de shell de la máquina donde Atlantis se está ejecutando. Use `--gh-app-key-file` para mitigar ese riesgo.
:::
...
Hostname de su instalación GitHub Enterprise. Si usa [GitHub.com](https://github.com),
no lo establezca. Por defecto es `github.com`.

Para GitHub Enterprise Cloud, use el hostname del tenant, por ejemplo `tenant.ghe.com`. No incluya un esquema ni un prefijo `api.`; Atlantis deriva los endpoints REST y GraphQL API a partir del hostname.
...
Nombre de organización de GitHub. Establézcalo para habilitar la creación de una GitHub app privada para esta organización.
...
En las versiones v0.20.1 y anteriores, el nombre del equipo de GitHub requería el nombre del equipo sensible a mayúsculas y minúsculas.

Lista separada por comas de equipos de GitHub y pares de permisos.

Por defecto, cualquier equipo puede plan y apply.

La jerarquía de equipos de GitHub se respeta. Si un equipo en la allowlist tiene equipos hijos, los miembros de esos equipos hijos heredan los comandos permitidos del equipo padre.

::: tip
Si está usando [policy checking](policy-checking.md), también debe incluir en la allowlist el comando `policy_check` para que funcione en comandos manuales `atlantis plan`:

```bash
atlantis server --gh-team-allowlist="*:plan,*:policy_check,myteam:apply"
```

Consulte [Policy Checking documentation](policy-checking.md#step-1-enable-the-workflow) para más detalles.
:::
...
Token de GitHub del usuario de API. El token se carga desde disco regularmente para permitir la rotación del token sin necesidad de reiniciar el servidor Atlantis.
...
Nombre de usuario de GitHub del usuario de API. Este usuario también es usado por el flag `--hide-user-plan-comments` y necesitará actualizarse si migra a github EMU.
...
URL base de la instalación de Gitea. Debe incluir `http://` o `https://`. Por defecto es `https://gitea.com` si se deja vacía/ausente.
...
Hostname de su instalación GitLab Enterprise. Si usa [GitLab.com](https://gitlab.com),
...
Ocultar comentarios de plan anteriores para despejar PRs. Esto solo está soportado en
GitHub, GitLab y Bitbucket actualmente y no está habilitado por defecto.

Para Bitbucket, los comentarios se eliminan en lugar de ocultarse ya que Bitbucket no soporta ocultar comentarios.

Para GitHub, asegúrese de que `--gh-user` esté configurado apropiadamente o los comentarios no se ocultarán.
...
Lista separada por comas de nombres de estado de VCS de otros servicios atlantis.
...
Usado por ejemplo con pre-workflow hooks de CDKTF que generan dinámicamente
archivos Terraform.
...
Atlantis normaliza valores de estilo locale (por ejemplo `es-MX` se resuelve a `es`).
Si se configura un idioma no soportado y `--language-config-file` no está establecido,
Atlantis devuelve un error de validación al inicio.
...
Ruta opcional a un catálogo de idioma YAML personalizado. Los valores de este archivo sobrescriben
...
y Atlantis recurre al inglés integrado para cadenas no especificadas.
...
- Si se establece en `redis`, use `--redis-host` e `--redis-port` para modo de nodo único, o `--redis-cluster-addresses` para modo Redis Cluster. Use `--redis-password` y (opcionalmente) `--redis-username` solo si su despliegue Redis requiere autenticación.
...
Las sobrescrituras de plantillas markdown pueden especificarse ya sea en archivos individuales, o todas juntas en un solo archivo. Todos los archivos de
...
Limitar el número de comentarios publicados después de que se ejecuta un comando, para evitar saturar su VCS y que Atlantis sea throttled como resultado. Por defecto es `100`. Establezca esta opción en `0` para deshabilitar el truncado de logs. Tenga en cuenta que el truncado ocurrirá en la parte superior de la salida del comando, para preservar las partes más importantes de la salida, que a menudo se muestran al final.

Cuando la salida del comando excede el límite de tamaño de comentario del VCS (o cuando este límite aplica), Atlantis divide la salida en múltiples comentarios usando **intelligent comment splitting**. Los puntos de división se eligen para preservar la estructura markdown: el divisor detecta si está dentro de un bloque de código (``
...
Tamaño máximo del wait group que ejecuta plans y applies en paralelo (si está habilitado). Por defecto es `15`
...
Establecer el estado del commit como pending cuando hay cambios planned que no se han aplicado.
...
por aplicar. Una vez que todos los proyectos se apliquen correctamente (o no muestren cambios), el estado se actualizará a success.
Los proyectos sin cambios Terraform se cuentan como actualizados en lugar de aplicados. Si un pull request tiene tanto
proyectos actualizados como proyectos todavía esperando apply, el estado de commit apply de Atlantis permanece pending
hasta que todos los proyectos cambiados se apliquen.
...
Puerto al que vincularse. Por defecto es `4141`.
...
Excluir comentarios de policy check de pull requests a menos que haya un error real de conftest. Esto también excluye advertencias. Por defecto es `false`.
...
Lista delimitada por comas de direcciones de nodos de cluster Redis en el formato `host:port`. Cuando se establece, Atlantis usa modo Redis Cluster en lugar de modo de nodo único. Esto es mutuamente excluyente con `--redis-host`/`--redis-port` (que se usan para modo de nodo único).
...
La base de datos Redis a usar cuando se usa un tipo Locking DB de `redis`. Por defecto es `0`.
...
El hostname Redis para cuando se usa un tipo Locking DB de `redis`.
...
La contraseña Redis para cuando se usa un tipo Locking DB de `redis`.
...
El puerto Redis para cuando se usa un tipo Locking DB de `redis`. Por defecto es `6379`.
...
Habilita una conexión TLS, con versión mínima 1.2, a Redis cuando se usa un tipo Locking DB de `redis`. Por defecto es `false`.
...
El nombre de usuario Redis para cuando se usa un tipo Locking DB de `redis`. Útil cuando Redis está configurado con autenticación basada en ACL.
...
Atlantis requiere que especifique una allowlist de repositorios desde los cuales aceptará webhooks.

Notas:

- Acepta una lista separada por comas, p. ej. `definition1,definition2`
- El formato es `{hostname}/{owner}/{repo}`, p. ej. `github.com/runatlantis/atlantis`
- `*` coincide con cualquier carácter, p. ej. `github.com/runatlantis/*` coincidirá con todos los repos en la organización runatlantis
- Una entrada que comienza con `!` la niega, p. ej. `github.com/foo/*,!github.com/foo/bar` coincidirá con todos los repos github en el owner `foo` _excepto_ `bar`.
- Para Bitbucket Server: `{hostname}` es el dominio sin esquema ni puerto, `{owner}` es el nombre del proyecto (no la clave), y `{repo}` es el nombre del repo
  - Los repositorios de usuario (no de proyecto) tienen el formato: `{hostname}/{full name}/{repo}` (p. ej., `bitbucket.example.com/Jane Doe/myatlantis` para nombre de usuario `jdoe` y nombre completo `Jane Doe`, lo que no es muy intuitivo)
...
- Microsoft está en proceso de cambiar Azure DevOps a la segunda forma, por lo que puede ser más seguro especificar siempre ambos formatos en su allowlist de repo para cada repositorio hasta que el cambio se complete.
...
Ruta a un archivo de configuración de repo YAML del lado del servidor. Consulte [Server Side Repo Config](server-side-repo-config.md).
...
Consulte [Server Side Repo Config](server-side-repo-config.md) para más detalles.
...
Cuando `--enable-regexp-cmd` también está habilitado, los plans de proyecto regex como `atlantis plan -p .*` se limitan a proyectos coincidentes con archivos modificados en el pull request.
Por defecto es `false`.
...
Directorio donde Atlantis almacenará archivos Terraform plan locales. Si no se establece, esto usa por defecto el valor `--data-dir` resuelto para que las instalaciones existentes mantengan el mismo layout en disco. Cuando se establece a un directorio diferente, los repositorios checkouteados permanecen bajo `--data-dir` y los archivos `.tfplan` generados usan el mismo layout de ruta de repo, pull request, workspace y proyecto bajo `--share-plan-dir`.
...
Algunos usuarios usan el flag `--repo-allowlist` para controlar a qué repos Atlantis
responde. Normalmente, si Atlantis recibe un webhook de pull request de un repo no listado
en la allowlist, responderá con un comentario de error. Este flag deshabilita ese comentario.
...
responderá con un comentario de error. Este flag deshabilita ese comentario.
...
`--silence-no-projects` le indicará a Atlantis que ignore PRs si ninguno de los archivos modificados forma parte de un proyecto definido en el archivo `atlantis.yaml`.
Este flag asegura que un servidor Atlantis solo responda a sus proyectos declarados explícitamente.
Esto no tiene efecto si los proyectos no están definidos en el `atlantis.yaml` a nivel de repo.
Esto también silencia comandos dirigidos (p. ej. `atlantis plan -d mydir` o `atlantis apply -p myproj`) de modo que si el proyecto no está en el repo config `atlantis.yaml`, estos comandos no se ejecutarán ni reportarán de vuelta en un comentario.

Esto es útil al ejecutar múltiples servidores Atlantis contra un único repositorio para que pueda
delegar trabajo a cada servidor Atlantis. También útil cuando se usa con pre_workflow_hooks para generar dinámicamente un archivo `atlantis.yaml`.
...
`--silence-vcs-status-no-plans` le indicará a Atlantis que ignore establecer estado VCS en plans si ninguno de los archivos modificados forma parte de un proyecto definido en el archivo `atlantis.yaml`.
...
`--silence-vcs-status-no-projects` le indicará a Atlantis que ignore establecer estado VCS en cualquier comando si ninguno de los archivos modificados forma parte de un proyecto definido en el archivo `atlantis.yaml`.
...
Token de API para notificaciones de Slack. Consulte [Using Slack hooks](sending-notifications-via-webhooks.md#using-slack-hooks).
...
Archivo que contiene certificado x509 usado para servir HTTPS.
...
Archivo que contiene clave privada x509 que coincide con `--ssl-cert-file`.
...
Namespace para emitir stats/metrics. Consulte la sección [stats](stats.md).
...
Por defecto es `true`. Permite a Atlantis listar y descargar versiones adicionales de Terraform.
Establecer esto en `false` puede ser útil en un entorno air-gapped donde no hay disponible un mirror de descarga.
...
donde releases.hashicorp.com no está disponible. La estructura de directorio del endpoint personalizado
...
Hostname de su instalación Terraform Enterprise para usar en conjunto con
`--tfe-token`. Consulte [Terraform Cloud](terraform-cloud.md) para más detalles.
Si usa Terraform Cloud (es decir, no tiene su propia instalación Terraform Enterprise)
no hace falta establecerlo ya que por defecto es `app.terraform.io`.
...
Habilítelo si está usando modo de ejecución local (en lugar del modo de ejecución remota de TFE/C). Consulte [Terraform Cloud](terraform-cloud.md) para más detalles.
...
Un token para integración Terraform Cloud/Terraform Enterprise. Consulte [Terraform Cloud](terraform-cloud.md) para más detalles.
...
Establézcalo en false si quiere deshabilitar terraform plugin cache.

Este flag es útil cuando hay múltiples proyectos que necesitan ejecutar un plan y apply en el mismo PR para evitar la condición de carrera de `plugin_cache_dir` concurrentemente, esto es un issue conocido de terraform, más información:
...
El efecto de la condición de carrera es más evidente al usar configuración paralela para ejecutar plan y apply. Deshabilitar el uso de plugin cache impactará el rendimiento al iniciar un nuevo plan o apply, pero en despliegues grandes de Atlantis con múltiples proyectos y módulos compartidos el uso de `--parallel_plan` y `--parallel_apply` es obligatorio para una gestión eficiente de los PRs.
...
Lista separada por comas de rutas de directorio adicionales desde las cuales pueden leerse [archivos de definición de variables](https://developer.hashicorp.com/terraform/language/values/variables#variable-definitions-tfvars-files).
Las rutas en este argumento deben ser rutas absolutas. Actualmente no se soportan rutas relativas ni globbing.
Si no se proporciona este argumento, por defecto será el directorio de datos de Atlantis, determinado por el argumento `--data-dir`.
...
Nombre usado para identificar Atlantis al actualizar un estado de pull request. Por defecto es `atlantis`.
...
Habilitar Basic Authentication en el servicio web de Atlantis.
...
Contraseña usada para Basic Authentication en el servicio web de Atlantis. Por defecto es `atlantis`.
...
Nombre de usuario usado para Basic Authentication en el servicio web de Atlantis. Por defecto es `atlantis`.
...
proporcionados como una cadena JSON. La clave del mapa es el nombre del header y el valor es el valor del header
...
Permitir conexión de websockets solo cuando se originan desde el servidor web Atlantis en ejecución
...
Escribir un archivo .git-credentials con el usuario y token del proveedor para permitir
clonar módulos privados por HTTPS o SSH. Consulte [Git Credential Store documentation](https://git-scm.com/docs/git-credential-store) para más información.
