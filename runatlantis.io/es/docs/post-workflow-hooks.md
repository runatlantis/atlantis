...
Los hooks post workflow se pueden definir para ejecutar scripts después de que se ejecuten workflows predeterminados o personalizados. Los hooks post workflow difieren de [custom
workflows](custom-workflows.md#custom-run-command) en que se ejecutan fuera de los comandos de Atlantis. Lo que significa que no muestran su salida de vuelta al PR como un comentario.

## Uso

Los hooks post workflow solo se pueden especificar en la Server-Side Repo Config bajo la clave `repos`.
...
Esto se puede modificar especificando la clave `commands` en el hook de workflow que contiene una lista delimitada por comas
 de comandos de Atlantis para los que el hook debe ejecutarse. El detalle de los comandos de Atlantis
se puede encontrar en [Using Atlantis](using-atlantis.md).
...
Puede agregar un hook post workflow para realizar informes personalizados después de que todos los workflows
hayan terminado.

En este ejemplo usamos un workflow personalizado para generar estimaciones de costos para cada
workflow usando [Infracost](https://www.infracost.io/docs/integrations/cicd/#cicd-integrations), luego crear un informe resumido después de que todos los workflows se hayan completado.
...
Esto es muy similar a [custom workflow run
...
| Clave | Tipo | Predeterminado | Requerido | Descripción |
| --- | --- | --- | --- | --- |
