...
* Alguien que agrega comentarios `atlantis plan/apply` en sus pull requests válidos, provocando que terraform se ejecute cuando usted no lo quiere.
...
Si está ejecutando en un repo público (lo cual no se recomienda, vea arriba) no debería establecer `--allow-fork-prs` (el valor predeterminado es false)
porque cualquiera puede abrir un pull request desde su fork a su repo.

### `--repo-allowlist`

Atlantis requiere que especifique una allowlist de repositorios desde los que aceptará webhooks mediante la flag `--repo-allowlist`.
Por ejemplo:
...
* Todos los repositorios: `--repo-allowlist=*`. Útil cuando está en una red protegida, pero peligroso sin establecer también un webhook secret.
...
Si los atacantes que envían pull requests con código Terraform malicioso están dentro de su modelo de amenazas,
...
1. Incluir providers en la imagen o host de Atlantis y denegar egress en producción.
1. Implementar internamente el protocolo del registro de providers y denegar egress público; de esa manera usted controla quién tiene acceso de escritura al registro.
1. Modificar el paso `plan` de su [configuración de repositorio del lado del servidor](server-side-repo-config.md) para validar contra el
   uso de providers o data sources no permitidos, o pull requests de usuarios no permitidos. También podría agregar validación extra en este punto, por ej.
...
solo a los archivos permitidos por la flag `--var-file-allowlist`. Si no se proporciona este argumento, su valor predeterminado es
...
Los webhook secrets aseguran que las solicitudes de webhook realmente provienen de su proveedor VCS (GitHub o GitLab).
...
Azure DevOps soporta enviar un encabezado de autenticación básica en todos los eventos de webhook. Esto requiere usar una URL HTTPS para la ubicación de su webhook.

### SSL/HTTPS

Si está usando webhook secrets pero su tráfico es por HTTP, entonces los webhook secrets
podrían ser robados. Habilite SSL/HTTPS usando las flags `--ssl-cert-file` e `--ssl-key-file`
.
...
También puede pasar estas como variables de entorno `ATLANTIS_WEB_BASIC_AUTH=true` `ATLANTIS_WEB_USERNAME=yourUsername` e `ATLANTIS_WEB_PASSWORD=yourPassword`.
