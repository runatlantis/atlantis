...
* `-w workspace` Cambia a este [Terraform workspace](https://developer.hashicorp.com/terraform/language/state/workspaces) antes de planificar. Por defecto es `default`. Ignore esto si los Terraform workspaces no se usan. Los nombres de workspace no pueden contener `/`, `\\`, `..`, `$`, espacios en blanco o caracteres de control, y no pueden comenzar con `-` o `~`.
* `--verbose` Agrega el log de Atlantis al comentario.

::: warning NOTE
Un `atlantis plan` (sin flags), como los autoplans, descarta todos los planes creados previamente con `atlantis plan` `-p`/`-d`/`-w`
:::
...
Si siempre necesita agregar un flag determinado, vea [Custom Workflow Use Cases](custom-workflows.md#adding-extra-arguments-to-terraform-commands).
...
Al ejecutar `atlantis plan`, Atlantis verifica automáticamente si hay un archivo en `env/{workspace}.tfvars` relativo al directorio del proyecto. Si este archivo existe, Atlantis lo incluirá automáticamente usando el flag `-var-file`.
...
* `atlantis plan` (usa el workspace por defecto) incluye automáticamente `env/default.tfvars`
...
::: warning NOTE
El flag `-destroy` genera un plan de destrucción. Si este plan se aplica, puede resultar en pérdida de datos o interrupciones del servicio. Asegúrese de haber revisado minuciosamente su configuración de Terraform y de tener la intención de eliminar los recursos especificados antes de usar este flag.
:::
...
::: tip
Si no se especifica ningún directorio/proyecto/workspace, ej. `atlantis apply`, este comando aplicará **todos los planes no aplicados de este pull request**.
Esto incluye todos los proyectos que se han planificado manualmente con `atlantis plan` `-p`/`-d`/`-w` desde el último autoplan o comando `atlantis plan`.
Para que los comandos de Atlantis funcionen, Atlantis necesita conocer la ubicación donde está el archivo del plan. Para eso, puede usar $PLANFILE que contendrá la ruta del archivo del plan que se usará en sus pasos personalizados. es decir `terraform plan -out $PLANFILE`
:::
...
* `-w workspace` Aplica el plan para este [Terraform workspace](https://developer.hashicorp.com/terraform/language/state/workspaces). Ignore esto si los Terraform workspaces no se usan. Los nombres de workspace no pueden contener `/`, `\\`, `..`, `$`, espacios en blanco o caracteres de control, y no pueden comenzar con `-` o `~`.
* `--auto-merge-disabled` Deshabilita [automerge](automerging.md) para este comando apply.
* `--auto-merge-method method` Especifica qué [merge method](automerging.md#how-to-set-the-merge-method-for-automerge) usar para el comando apply si [automerge](automerging.md) está habilitado. Implementado solo para GitHub.
* `--verbose` Agrega el log de Atlantis al comentario.

### Flags adicionales de Terraform

Debido a que Atlantis internamente está ejecutando `terraform apply plan.tfplan`, cualquier opción de Terraform que cambie el `plan` se ignora, ej.:
...
Si desea especificar estos flags, hágalo al ejecutar `atlantis plan`.
...
Cancela todos los **comandos en cola** del pull request actual.
...
Este comando descarta el resultado del terraform plan. Después de un import y antes de un apply, otro `atlantis plan` debe ejecutarse de nuevo.
...
* `-w workspace` Importa un recurso para un [Terraform workspace](https://developer.hashicorp.com/terraform/language/state/workspaces) específico. Ignore esto si los Terraform workspaces no se usan. Los nombres de workspace no pueden contener `/`, `\\`, `..`, `$`, espacios en blanco o caracteres de control, y no pueden comenzar con `-` o `~`.

### Flags adicionales de Terraform

Si `terraform import` requiere argumentos adicionales, como `-var 'foo=bar'` o `-var-file myfile.tfvars`
agréguelos al final del comentario después de `--`, p. ej.

```shell
atlantis import -d dir 'aws_instance.example["foo"]' i-1234567890abcdef0 -- -var foo='bar'
```

Si se necesita que un flag se agregue siempre, vea [Custom Workflow Use Cases](custom-workflows.md#adding-extra-arguments-to-terraform-commands).
...
Este comando descarta el resultado del terraform plan. Después de ejecutar `state rm` y antes de un apply, otro `atlantis plan` debe ejecutarse de nuevo.
...
* `-d directory` Ejecuta state rm para un recurso en este directorio, relativo a la raíz del repo. Use `.` para la raíz.
* `-p project` Ejecuta state rm para un recurso en este proyecto. Se refiere al nombre del proyecto configurado en el archivo de configuración del repo [`atlantis.yaml`](repo-level-atlantis-yaml.md). Esto no puede usarse al mismo tiempo que `-d` o `-w`.
* `-w workspace` Ejecuta state rm para un recurso en un [Terraform workspace](https://developer.hashicorp.com/terraform/language/state/workspaces) específico. Ignore esto si los Terraform workspaces no se usan. Los nombres de workspace no pueden contener `/`, `\\`, `..`, `$`, espacios en blanco o caracteres de control, y no pueden comenzar con `-` o `~`.
...
Además de los comentarios de pull request, Atlantis soporta workflows basados en API para plan, apply y detección de drift. Estos endpoints permiten que herramientas externas y automatización interactúen con Atlantis programáticamente.

Capacidades clave:

* **Plan and Apply** sin un pull request (`POST /api/plan`, `POST /api/apply`)
* **Drift Detection** para identificar cambios de infraestructura fuera de Terraform (`POST /api/drift/detect`)
* **Drift Status** para ver resultados de drift en caché (`GET /api/drift/status`)
* **Drift Remediation** para corregir el drift detectado (`POST /api/drift/remediate`)
