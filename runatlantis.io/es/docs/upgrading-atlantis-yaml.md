...
El único cambio de v2 a v3 es que estamos analizando los pasos custom de `run` de manera diferente.
...
<center><i>Un ejemplo de workflow usando un paso run custom</i></center>

Anteriormente, usábamos una biblioteca que analizaba el paso custom antes de ejecutarlo.
Ahora, simplemente ejecutamos el paso directamente. Esto solo afectará a tus pasos si estaban usando algún tipo de escape de shell.
...
Y tu `atlantis.yaml` se vería algo como:
...
Para empezar, determina si estás personalizando comandos que ocurren durante
...
antes/después de los comandos integrados. Dada una configuración anterior:
