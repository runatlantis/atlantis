...
**Si no estás usando pasos personalizados de [custom `run`](custom-workflows.md#custom-run-command),
 entonces puedes actualizar de `version: 2` a `version: 3` sin ningún cambio.**

**NOTA:** La versión 2 **no está siendo desaprobada** y no hay necesidad de actualizar tu versión
si no deseas hacerlo.

El único cambio de v2 a v3 es que estamos analizando los pasos personalizados de `run` de manera diferente.
...
<center><i>Un ejemplo de workflow usando un paso run personalizado</i></center>

Anteriormente, usábamos una biblioteca que analizaba el paso personalizado antes de ejecutarlo.
Ahora, simplemente ejecutamos el paso directamente. Esto solo afectará tus pasos si estaban usando algún tipo de escape de shell.
...
Los workflows son la nueva forma de establecer todos los `pre_*`, `post_*` y `extra_arguments`.

Cada `project` puede tener un workflow personalizado mediante la clave `workflow`.
...
`extra_arguments` ahora se especifica de la siguiente manera. Dada una configuración anterior:
...
En lugar de usar `pre_*` o `post_*`, ahora puedes insertar tus comandos personalizados
antes/después de los comandos incorporados. Dada una configuración anterior:
...
::: tip
Es importante incluir los comandos incorporados: `init`, `plan` y `apply`.
