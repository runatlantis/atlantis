...
const (
	...
	AllowUnverifiedWebhookSignaturesFlag = "allow-unverified-webhook-signatures"
...
var boolFlags = map[string]boolFlag{
	...
	AllowUnverifiedWebhookSignaturesFlag: {
		description: "Accept webhook requests that carry a signature or token header even when Atlantis has no webhook secret configured to validate them. " +
			"By default such requests are rejected because they indicate a secret was configured on the VCS host but not in Atlantis.",
		defaultValue: false,
	},
	AutoplanModules: {
