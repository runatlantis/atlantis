...
var lockDeletedTemplate = template.Must(template.New("").Parse(
	"The following Locks and plans deleted were deleted from the Atlantis UI:\n" +
		"{{ range . }}\n" +
...
func (l *LockController) DeleteLockRoute(w http.ResponseWriter, r *http.Request) error {
	...
	if err != nil {
		l.respond(w, logging.Warn, http.StatusBadRequest, "Failed to delete lock id: %s. Failed with %s")
		return nil
	}
	return nil
}
...
func (l *LockController) CommentOnPullRequest(lock models.ProjectLock) error {
	...
	return nil
	//return l.VCSClient.CreateComment(lock.Pull.Repo, lock.Pull.Num, buf.String(), lock.Pull.Repo.Hostname)
}
