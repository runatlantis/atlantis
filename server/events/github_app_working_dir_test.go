...
func TestClone_GithubAppSetsCorrectUrl(t *testing.T) {
	...
	When(workingDir.Clone(logger, modifiedBaseRepo, models.PullRequest{BaseRepo: modifiedBaseRepo})).ThenReturn(
		"", true, nil,
	)

	_, success, _ := ghAppWorkingDir.Clone(logger, headRepo, models.PullRequest{BaseRepo: baseRepo})
