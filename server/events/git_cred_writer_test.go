...
func TestWriteGitCreds_WriteFile(t *testing.T) {
		...
		err := writeGitCreds("user", "token", "hostname", tmp)
		Ok(t, err)

		expContents := `user:token@hostname`
...
func TestWriteGitCreds_WillNotOverwrite(t *testing.T) {
		...
		actErr := writeGitCreds("user", "token", "hostname", tmp)
		expError := fmt.Sprintf("can't write git-credentials to %s because that file has contents that would be overwritten", tmp+"/.git-credentials")
...
func TestWriteGitCreds_ErrIfCannotWrite(t *testing.T) {
		...
		actErr := writeGitCreds("user", "token", "hostname", "/this/dir/does/not/exist")
		ErrEquals(t, expErr, actErr)
}
