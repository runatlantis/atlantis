...
func (mock *MockWorkingDir) Clone(log logging.SimpleLogging, headRepo models.Repo, p models.PullRequest, workspace string, additionalBranches []string) (string, bool, error) {
	if mock == nil {
		panic("mock must not be nil. Use myMock := NewMockWorkingDir().")
	}
	params := []pegomock.Param{log, headRepo, p, workspace, additionalBranches}
...
func (verifier *VerifierMockWorkingDir) Clone(log logging.SimpleLogging, headRepo models.Repo, p models.PullRequest, workspace string, additionalBranches []string) *MockWorkingDir_Clone_OngoingVerification {
	params := []pegomock.Param{log, headRepo, p, workspace, additionalBranches}
...
func (c *MockWorkingDir_Clone_OngoingVerification) GetCapturedArguments() (logging.SimpleLogging, models.Repo, models.PullRequest, string, []string) {
	log, headRepo, p, workspace, additionalBranches := c.GetAllCapturedArguments()
	return log[len(log)-1], headRepo[len(headRepo)-1], p[len(p)-1], workspace[len(workspace)-1], additionalBranches[len(additionalBranches)-1]
}
