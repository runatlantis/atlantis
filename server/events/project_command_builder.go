...
type ProjectApplyCommandBuilder interface {
	ProjectTargetedDirIgnorer
	// ShouldSilenceTargetedApply reports whether the existing repo config can
	// exclude a named apply without building commands or acquiring locks.
	ShouldSilenceTargetedApply(ctx *command.Context, comment *CommentCommand) bool
...
// ShouldSilenceTargetedApply checks the existing default workspace config, which
// also contains any configuration generated by pre-workflow hooks. Without plan
// status for the requested head and base, the checkout may still describe an
// earlier revision and must follow the normal apply path.
func (p *DefaultProjectCommandBuilder) ShouldSilenceTargetedApply(ctx *command.Context, cmd *CommentCommand) bool {
	if !p.SilenceNoProjects || cmd.ProjectName == "" {
		return false
	}
	if ctx.PullStatus == nil || ctx.Pull.HeadCommit == "" || ctx.Pull.BaseBranch == "" ||
		ctx.PullStatus.Pull.HeadCommit != ctx.Pull.HeadCommit || ctx.PullStatus.Pull.BaseBranch != ctx.Pull.BaseBranch {
		return false
	}
	...
	projects, repoCfg, err := p.getCfg(ctx, cmd.ProjectName, repoRelDir, workspace, repoDir,
		ctx.API && cmd.RepoRelDir != "", ctx.API && cmd.Workspace != "")
	// Preserve normal error handling for missing, empty, or invalid configs.
	return err == nil && repoCfg != nil && len(repoCfg.Projects) > 0 && len(projects) == 0
}
