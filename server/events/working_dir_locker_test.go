...
func TestTryLock(t *testing.T) {
	...
	unlockFn, err := locker.TryLock(repo, 1, workspace, path, cmd)
	Ok(t, err)
	...
	// Unlock should work.
	unlockFn()
	_, err = locker.TryLock(repo, 1, workspace, path, cmd)
	Ok(t, err)
}
...
func TestTryLockSameCommand(t *testing.T) {
	...
	// The first lock should succeed.
	unlockFn, err := locker.TryLock(repo, 1, workspace, path, command.Import)
	Ok(t, err)
...
func TestTryLockDifferentWorkspaces(t *testing.T) {
	...
	_, err := locker.TryLock(repo, 1, workspace, path, cmd)
	Ok(t, err)
	_, err = locker.TryLock(repo, 1, "new-workspace", path, cmd)
	Ok(t, err)
...
func TestTryLockDifferentRepo(t *testing.T) {
	...
	_, err = locker.TryLock(newRepo, 1, workspace, path, cmd)
	Ok(t, err)
...
func TestTryLockDifferentPulls(t *testing.T) {
	...
	Ok(t, err)
	newPull := 2
	_, err = locker.TryLock(repo, newPull, workspace, path, cmd)
	Ok(t, err)
...
func TestTryLockDifferentPaths(t *testing.T) {
	...
	Ok(t, err)
	newPath := "new-path"
	_, err = locker.TryLock(repo, 1, workspace, newPath, cmd)
	Ok(t, err)
...
func TestUnlock(t *testing.T) {
	...
	Ok(t, err)
	unlockFn()
	_, err = locker.TryLock(repo, 1, workspace, "", cmd)
	Ok(t, err)
}
...
func TestUnlockDifferentWorkspaces(t *testing.T) {
	...
	unlockFn1, err1 := locker.TryLock(repo, 1, workspace, path, cmd)
	Ok(t, err1)
	unlockFn2, err2 := locker.TryLock(repo, 1, "new-workspace", path, cmd)
	Ok(t, err2)
	unlockFn1()
	unlockFn2()
...
func TestUnlockDifferentRepos(t *testing.T) {
	...
	Ok(t, err1)
	newRepo := "owner/newrepo"
	unlockFn2, err2 := locker.TryLock(newRepo, 1, workspace, path, cmd)
	Ok(t, err2)
	unlockFn1()
	unlockFn2()
...
func TestUnlockDifferentPulls(t *testing.T) {
	...
	Ok(t, err1)
	newPull := 2
	unlockFn2, err2 := locker.TryLock(repo, newPull, workspace, path, cmd)
	Ok(t, err2)
	unlockFn1()
	unlockFn2()
