...
func TestAggregateApplyRequirements_ValidatePlanProject(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			setup: func(workingDir *mocks.MockWorkingDir) {
				When(workingDir.HasDiverged(matchers.AnyLoggingSimpleLogging(), Any[string]())).ThenReturn(false)
			},
			wantErr: assert.NoError,
		},
		{
		...
		},
		{
			...
			setup: func(workingDir *mocks.MockWorkingDir) {
				When(workingDir.HasDiverged(matchers.AnyLoggingSimpleLogging(), Any[string]())).ThenReturn(true)
			},
