...
type ProjectContext struct {
	...
	// ValidatedPlanFilePath is an internal, command-scoped snapshot used to bind
	// managed custom apply steps to the exact bytes accepted by validation.
	ValidatedPlanFilePath string
	// RecordedManagedPlanHash is the authoritative SHA-256 digest copied from
	// durable pull status for apply validation. It is distinct from
	// ExpectedPlanHash, which identifies the local file selected at command start.
	RecordedManagedPlanHash string
	// AcceptedPlanGeneration identifies the durable plan generation that
	// authorized this apply and may own cleanup of its persisted artifact.
	AcceptedPlanGeneration string
	// PlanGeneration identifies the plan attempt that owns this command.
	PlanGeneration string
	// GeneratedPlanHash receives the SHA-256 hash of the convention-managed
	// plan artifact generated by the plan step. The pointer is shared by copies
	// of this context so the project command result can persist the exact hash.
	...
	// RemotePlanRunURL receives the Terraform Cloud/Enterprise run URL found by
	// remote plan execution so deferred final status publication can use it only
	// after durable plan-generation completion.
	RemotePlanRunURL *string
