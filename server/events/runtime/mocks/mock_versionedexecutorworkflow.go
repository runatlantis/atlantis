...
func (mock *MockVersionedExecutorWorkflow) ResolveArgs(ctx models.ProjectCommandContext) ([]string, error) {
	if mock == nil {
		panic("mock must not be nil. Use myMock := NewMockVersionedExecutorWorkflow().")
	}
	params := []pegomock.Param{ctx}
	result := pegomock.GetGenericMockFrom(mock).Invoke("ResolveArgs", params, []reflect.Type{reflect.TypeOf((*[]string)(nil)).Elem(), reflect.TypeOf((*error)(nil)).Elem()})
	var ret0 []string
	var ret1 error
	if len(result) != 0 {
		if result[0] != nil {
...
func (mock *MockVersionedExecutorWorkflow) Run(log *logging.SimpleLogger, executablePath string, envs map[string]string, args []string) (string, error) {
	if mock == nil {
		panic("mock must not be nil. Use myMock := NewMockVersionedExecutorWorkflow().")
	}
	params := []pegomock.Param{log, executablePath, envs, args}
...
func (verifier *VerifierMockVersionedExecutorWorkflow) ResolveArgs(ctx models.ProjectCommandContext) *MockVersionedExecutorWorkflow_ResolveArgs_OngoingVerification {
	params := []pegomock.Param{ctx}
	methodInvocations := pegomock.GetGenericMockFrom(verifier.mock).Verify(verifier.inOrderContext, verifier.invocationCountMatcher, "ResolveArgs", params, verifier.timeout)
	return &MockVersionedExecutorWorkflow_ResolveArgs_OngoingVerification{mock: verifier.mock, methodInvocations: methodInvocations}
}

type MockVersionedExecutorWorkflow_ResolveArgs_OngoingVerification struct {
	mock              *MockVersionedExecutorWorkflow
	methodInvocations []pegomock.MethodInvocation
}

func (c *MockVersionedExecutorWorkflow_ResolveArgs_OngoingVerification) GetCapturedArguments() models.ProjectCommandContext {
	ctx := c.GetAllCapturedArguments()
	return ctx[len(ctx)-1]
}

func (c *MockVersionedExecutorWorkflow_ResolveArgs_OngoingVerification) GetAllCapturedArguments() (_param0 []models.ProjectCommandContext) {
	params := pegomock.GetGenericMockFrom(c.mock).GetInvocationParams(c.methodInvocations)
	if len(params) > 0 {
...
func (verifier *VerifierMockVersionedExecutorWorkflow) Run(log *logging.SimpleLogger, executablePath string, envs map[string]string, args []string) *MockVersionedExecutorWorkflow_Run_OngoingVerification {
	params := []pegomock.Param{log, executablePath, envs, args}
...
func (c *MockVersionedExecutorWorkflow_Run_OngoingVerification) GetCapturedArguments() (*logging.SimpleLogger, string, map[string]string, []string) {
	log, executablePath, envs, args := c.GetAllCapturedArguments()
	return log[len(log)-1], executablePath[len(executablePath)-1], envs[len(envs)-1], args[len(args)-1]
}

func (c *MockVersionedExecutorWorkflow_Run_OngoingVerification) GetAllCapturedArguments() (_param0 []*logging.SimpleLogger, _param1 []string, _param2 []map[string]string, _param3 [][]string) {
	params := pegomock.GetGenericMockFrom(c.mock).GetInvocationParams(c.methodInvocations)
	if len(params) > 0 {
		_param0 = make([]*logging.SimpleLogger, len(c.methodInvocations))
		for u, param := range params[0] {
