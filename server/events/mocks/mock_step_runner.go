...
func (mock *MockStepRunner) Run(ctx models.ProjectCommandContext, extraArgs []string, path string, envs map[string]string, parallel models.ParallelCommand) (string, error) {
	if mock == nil {
		panic("mock must not be nil. Use myMock := NewMockStepRunner().")
	}
	params := []pegomock.Param{ctx, extraArgs, path, envs, parallel}
...
func (verifier *VerifierMockStepRunner) Run(ctx models.ProjectCommandContext, extraArgs []string, path string, envs map[string]string, parallel models.ParallelCommand) *MockStepRunner_Run_OngoingVerification {
	params := []pegomock.Param{ctx, extraArgs, path, envs, parallel}
...
func (c *MockStepRunner_Run_OngoingVerification) GetCapturedArguments() (models.ProjectCommandContext, []string, string, map[string]string, models.ParallelCommand) {
	ctx, extraArgs, path, envs, parallel := c.GetAllCapturedArguments()
	return ctx[len(ctx)-1], extraArgs[len(extraArgs)-1], path[len(path)-1], envs[len(envs)-1], parallel[len(parallel)-1]
}

func (c *MockStepRunner_Run_OngoingVerification) GetAllCapturedArguments() (_param0 []models.ProjectCommandContext, _param1 [][]string, _param2 []string, _param3 []map[string]string, _param4 []models.ParallelCommand) {
	...
	if len(params) > 0 {
		...
		_param4 = make([]models.ParallelCommand, len(c.methodInvocations))
		for u, param := range params[4] {
			_param4[u] = param.(models.ParallelCommand)
		}
	}
	return
}
