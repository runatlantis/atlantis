...
func (mock *MockCommentParsing) Parse(comment string, vcsHost models.Host) events.CommentParseResult {
...
func (verifier *VerifierCommentParsing) Parse(comment string, vcsHost models.Host) *CommentParsing_Parse_OngoingVerification {
	params := []pegomock.Param{comment, vcsHost}
...
func (c *CommentParsing_Parse_OngoingVerification) GetCapturedArguments() (string, models.Host) {
	comment, vcsHost := c.GetAllCapturedArguments()
	return comment[len(comment)-1], vcsHost[len(vcsHost)-1]
}

func (c *CommentParsing_Parse_OngoingVerification) GetAllCapturedArguments() (_param0 []string, _param1 []models.Host) {
	...
	if len(params) > 0 {
		...
		_param1 = make([]models.Host, len(params[1]))
		for u, param := range params[1] {
			_param1[u] = param.(models.Host)
		}
	}
	return
}
