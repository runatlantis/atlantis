...
func TestNewCommand_AllFieldsSet(t *testing.T) {
	cmd := events.NewCommentCommand("dir", []string{"a", "b"}, command.Plan, "", true, false, "", "workspace", "project", "mygroup", "policyset", false)
	Equals(t, events.CommentCommand{
		Workspace:   "workspace",
		RepoRelDir:  "dir",
		Verbose:     true,
...
func TestCommentCommand_String(t *testing.T) {
	exp := `command="plan", verbose=true, dir="mydir", workspace="myworkspace", project="myproject", group="mygroup", policyset="", auto-merge-disabled=false, auto-merge-method=, clear-policy-approval=false, flags="flag1,flag2"`
