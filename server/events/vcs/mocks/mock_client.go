...
func (mock *MockClient) UpdateStatus(ctx context.Context, request types.UpdateStatusRequest) error {
	if mock == nil {
		panic("mock must not be nil. Use myMock := NewMockClient().")
	}
	params := []pegomock.Param{ctx, request}
...
func (verifier *VerifierMockClient) UpdateStatus(ctx context.Context, request types.UpdateStatusRequest) *MockClient_UpdateStatus_OngoingVerification {
	params := []pegomock.Param{ctx, request}
...
func (c *MockClient_UpdateStatus_OngoingVerification) GetCapturedArguments() (context.Context, types.UpdateStatusRequest) {
	ctx, request := c.GetAllCapturedArguments()
	return ctx[len(ctx)-1], request[len(request)-1]
}
