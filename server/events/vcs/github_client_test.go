...
func TestGithubClient_GetModifiedFiles(t *testing.T) {
	...
	respTemplate := `[
  {
	...
	client, err := vcs.NewGithubClient(testServerURL.Host, &vcs.GithubUserCredentials{"user", "pass"}, logger, mergeabilityChecker, false)
	Ok(t, err)
...
func TestGithubClient_GetModifiedFilesMovedFile(t *testing.T) {
	resp := `[
  {
	...
	Ok(t, err)
	mergeabilityChecker := vcs.NewPullMergeabilityChecker("atlantis")
	client, err := vcs.NewGithubClient(testServerURL.Host, &vcs.GithubUserCredentials{"user", "pass"}, logging.NewNoopLogger(t), mergeabilityChecker, false)
	Ok(t, err)
	defer disableSSLVerification()()
...
func TestGithubClient_MarkdownPullLink(t *testing.T) {
	mergeabilityChecker := vcs.NewPullMergeabilityChecker("atlantis")
	client, err := vcs.NewGithubClient("hostname", &vcs.GithubUserCredentials{"user", "pass"}, logging.NewNoopLogger(t), mergeabilityChecker, false)
	Ok(t, err)
