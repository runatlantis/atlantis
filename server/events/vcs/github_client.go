...
func NewGithubClient(hostname string, credentials GithubCredentials, logger logging.SimpleLogging, mergeabilityChecker MergeabilityChecker, useGithubChecks bool) (*GithubClient, error) {
	...
	if useGithubChecks {
		statusUpdater = &GithubStatusCheckUpdater{client}
	} else {
		statusUpdater = &PullStatusCheckUpdater{client}
	}
