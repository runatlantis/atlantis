...
func (g *GitlabClient) CreateComment(logger logging.SimpleLogging, repo models.Repo, pullNum int, comment string, command string) error {
	logger.Debug("Creating comment on GitLab merge request %d", pullNum)
	comments := common.SplitComment(logger, comment, gitlabMaxCommentLength, 0, command)
