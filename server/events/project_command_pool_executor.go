...
func runApplyCmdsWithOptionalReplan(
	...
	for i, group := range groups {
		...
		// Historical abort_on_execution_order_fail behavior, kept ahead of the
		// status update so the feature-off path stays identical to upstream.
		if !replanEnabled && groupResult.HasErrors() && group[0].AbortOnExecutionOrderFail && isParallel {
			ctx.Log.Info("abort on execution order when failed")
			break
		}
