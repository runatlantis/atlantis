...
func (mock *MockEventParsing) DetermineCommand(comment *github.IssueCommentEvent) (*Command, error) {
	params := []pegomock.Param{comment}
	result := pegomock.GetGenericMockFrom(mock).Invoke("DetermineCommand", params, []reflect.Type{reflect.TypeOf((**Command)(nil)).Elem(), reflect.TypeOf((*error)(nil)).Elem()})
	var ret0 *Command
	var ret1 error
	if len(result) != 0 {
		if result[0] != nil {
			ret0 = result[0].(*Command)
		}
		if result[1] != nil {
			ret1 = result[1].(error)
		}
...
func (mock *MockEventParsing) ExtractCommentData(comment *github.IssueCommentEvent, ctx *CommandContext) error {
...
func (verifier *VerifierEventParsing) ExtractCommentData(comment *github.IssueCommentEvent, ctx *CommandContext) *EventParsing_ExtractCommentData_OngoingVerification {
	params := []pegomock.Param{comment, ctx}
...
func (c *EventParsing_ExtractCommentData_OngoingVerification) GetCapturedArguments() (*github.IssueCommentEvent, *CommandContext) {
	comment, ctx := c.GetAllCapturedArguments()
	return comment[len(comment)-1], ctx[len(ctx)-1]
}

func (c *EventParsing_ExtractCommentData_OngoingVerification) GetAllCapturedArguments() (_param0 []*github.IssueCommentEvent, _param1 []*CommandContext) {
	params := pegomock.GetGenericMockFrom(c.mock).GetInvocationParams(c.methodInvocations)
	if len(params) > 0 {
		_param0 = make([]*github.IssueCommentEvent, len(params[0]))
		for u, param := range params[0] {
		...
		_param1 = make([]*CommandContext, len(params[1]))
		for u, param := range params[1] {
			_param1[u] = param.(*CommandContext)
		}
	}
	return
}
