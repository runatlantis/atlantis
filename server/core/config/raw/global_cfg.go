...
type GlobalCfg struct {
	...
	Stores     Stores              `yaml:"stores" json:"stores"`
}
...
func (g GlobalCfg) Validate() error {
	...
	if err := g.Backends.Validate(); err != nil {
		return err
	}

	if err := g.Stores.validate(g.Backends); err != nil {
		return err
	}
...
func (g GlobalCfg) ToValid(defaultCfg valid.GlobalCfg) valid.GlobalCfg {
	...
	return valid.GlobalCfg{
		...
		Backends:   g.Backends.ToValid(),
		Stores:     g.Stores.ToValid(),
	}
}
