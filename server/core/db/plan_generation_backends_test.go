...
type generationBackend interface {
	DiscardPlanStatus(models.PullRequest, models.ProjectStatus, command.PublicationWriteMode) (bool, error)
	UpdateProjectStatus(models.PullRequest, string, string, models.ProjectPlanStatus, command.PublicationWriteMode) error

	BeginPlanGeneration(models.PullRequest, string, []command.ProjectContext, bool, command.PublicationWriteMode) (db.PlanGenerationBeginResult, error)
	UpdatePullWithResults(models.PullRequest, []command.ProjectResult, command.PublicationWriteMode) (models.PullStatus, error)
	GetPullStatus(models.PullRequest) (*models.PullStatus, error)
	DeletePullStatus(models.PullRequest, command.PublicationWriteMode) error
	Close() error
}
...
func TestPlanGeneration_BackendParity(t *testing.T) {
	...
	for name, newBackend := range backends {
		t.Run(name, func(t *testing.T) {
			...
			_, err := b.BeginPlanGeneration(pull, "G1", []command.ProjectContext{generationProject("a")}, true, command.NoClaim{})
			Ok(t, err)
			_, err = b.BeginPlanGeneration(pull, "G2", []command.ProjectContext{generationProject("a")}, true, command.NoClaim{})
			Ok(t, err)
			accepted, err := b.UpdatePullWithResults(pull, []command.ProjectResult{generationResult("a", "G2")}, command.NoClaim{})
			Ok(t, err)
			_, err = b.UpdatePullWithResults(pull, []command.ProjectResult{generationResult("a", "G1")}, command.NoClaim{})
			...
			_, err = b.UpdatePullWithResults(pull, []command.ProjectResult{generationResult("a", "")}, command.NoClaim{})
			...
			err = b.UpdateProjectStatus(pull, "default", ".", models.DiscardedPlanStatus, command.NoClaim{})
			...
			_, err = b.DiscardPlanStatus(pull, wrong, command.NoClaim{})
			...
			_, err = b.UpdatePullWithResults(pull, []command.ProjectResult{applied}, command.NoClaim{})
			...
			_, err = b.BeginPlanGeneration(pull, "G3", []command.ProjectContext{generationProject("a")}, true, command.NoClaim{})
			Ok(t, err)
			_, err = b.UpdatePullWithResults(pull, []command.ProjectResult{generationResult("a", "G3")}, command.NoClaim{})
			Ok(t, err)
			...
			actual, err = b.GetPullStatus(pull)
			Ok(t, err)
			discarded, err := b.DiscardPlanStatus(pull, actual.Projects[0], command.NoClaim{})
			Ok(t, err)
			...
			Ok(t, b.DeletePullStatus(pull, command.NoClaim{}))
			_, err = b.UpdatePullWithResults(pull, []command.ProjectResult{generationResult("a", "G2")}, command.NoClaim{})
			...
			_, err = b.DiscardPlanStatus(pull, models.ProjectStatus{Workspace: "default", RepoRelDir: ".", ProjectName: "a"}, command.NoClaim{})
