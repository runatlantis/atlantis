	...
	for name, create := range backends {
		t.Run(name, func(t *testing.T) {
			...
			for _, mode := range []command.PublicationWriteMode{command.NoClaim{}, command.PublicationFence{Owner: "wrong"}} {
				...
				require.ErrorIs(t, database.DiscardPullPlans(pull, &initial, mode), expected)
			}
			...
			require.ErrorIs(t, database.DiscardPullPlans(pull, &accepted, fence), db.ErrPublicationAmbiguous)
			...
			require.ErrorIs(t, database.DiscardPullPlans(pull, nil, fence), db.ErrPublicationOwnerLost)
			replacement := command.PublicationFence{Owner: "replacement"}
			// A missing status still requires the exact fence. The empty durable
			// result must not authorize cleanup of a later plan generation.
			require.NoError(t, database.DiscardPullPlans(pull, nil, replacement))
			empty, err := database.GetPullStatus(pull)
			require.NoError(t, err)
			require.Empty(t, empty.Projects)
			_, err = database.BeginPlanGeneration(pull, "G2", []command.ProjectContext{generationProject("a")}, false, replacement)
			require.NoError(t, err)
			accepted, err = database.UpdatePullWithResults(pull, []command.ProjectResult{generationResult("a", "G2")}, replacement)
			require.NoError(t, err)
			require.ErrorIs(t, database.DiscardPullPlans(pull, empty, replacement), db.ErrPlanGenerationSuperseded)
			require.ErrorIs(t, database.DiscardPullPlans(pull, nil, replacement), db.ErrPlanGenerationSuperseded)
			require.NoError(t, database.DiscardPullPlans(pull, &accepted, replacement))
			discarded, err := database.GetPullStatus(pull)
			require.NoError(t, err)
			require.Equal(t, models.DiscardedPlanStatus, discarded.Projects[0].Status)
			require.Empty(t, discarded.Projects[0].AcceptedPlanGeneration)
...
func TestPublicationLease_RedisExpiryUsesServerClock(t *testing.T) {
	...
	require.ErrorIs(t, store.DiscardPullPlans(pull, &initial, command.PublicationFence{Owner: "owner"}), db.ErrPublicationOwnerLost)
	...
	require.ErrorIs(t, store.DiscardPullPlans(pull, &initial, command.NoClaim{}), db.ErrPublicationAmbiguous)
