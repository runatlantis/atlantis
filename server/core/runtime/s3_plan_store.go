...
func (s *S3PlanStore) RestorePlans(pullDir, owner, repo string, pullNum int) error {
	...
	for {
		...
		for _, obj := range resp.Contents {
			...
			// Verify the resolved path is still within pullDir to prevent path traversal.
			absLocal, err := filepath.Abs(localPath)
			if err != nil {
				return fmt.Errorf("resolving absolute path for %s: %w", localPath, err)
			}
			absPull, err := filepath.Abs(pullDir)
			if err != nil {
				return fmt.Errorf("resolving absolute path for pull dir: %w", err)
			}
			if !strings.HasPrefix(absLocal, absPull+string(os.PathSeparator)) {
				return fmt.Errorf("refusing to restore plan outside pull dir (resolved=%s, pullDir=%s)", absLocal, absPull)
			}
